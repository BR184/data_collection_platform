...
- 涉及 BI 看板：先读取 `docs/bi-dashboard/README.md` 和 `docs/bi-dashboard/progress.md`，再按入口路由读取并更新对应职责文件；BI 专属事实只维护在该目录，通用平台文档只允许保留入口引用。
- 涉及 BI 字段、指标、图表或下载中的任何数据计算：必须读取并直接遵循 `docs/bi-dashboard/BI看板数据来源与计算口径核对表.md` 的对应编号；该表是用户人工确认的业务语义与公式权威，其中英文候选字段名不自动构成物理字段契约。实现时必须从来源表结构、实体与查询核实真实 `table.column`，对语义一致但名称不同的字段记录“等价映射”；只有业务语义或公式冲突时才停止接入并报告，不得因字段名不一致拒绝已有数据，也不得增加别名、回退或双轨。
- 涉及 CAT 集成测试接入：必须同时读取 `docs/bi-dashboard/集成测试平台接口使用手册.md` 和 `docs/bi-dashboard/data-contracts.md`；前者是 CAT 已提供接口的原始事实，后者是 BI 映射、完整性和错误边界。手册未说明的认证、快照、字段或公式不得自行推断。
...
- 涉及后端接口或平台产出行为的新增、修改、删除：遵循本文件“黄金基线回归门禁”章节；机制细节见 `docs/architecture.md`“黄金基线回归测试”章节。
...
- 测试必须直击当前缺陷的真实运行逻辑和用户验收路径；本地数据规模不足以代表目标容量时，不得用小规模数据做无意义的容量替代测试或外推结论，容量问题必须留给目标内网负载实测。优先验证真实新增、更新、删除输入是否按自动链路进入 ODS、事实和投影，以及既有数据是否保持正确。
...
平台产出行为由黄金基线全量锁定：冻结夹具走真实链路（同步→事实→统计）后，全部产出端点的响应、导出与受影响表状态和冻结快照严格比对，差异即回归。机制详见 `docs/architecture.md`“黄金基线回归测试”章节与 `docs/decisions.md` D-08；本节只定义运行时机与增删改纪律。

- 运行时机：全链路约 7 分钟（双 PostgreSQL 容器 + 从零建库 + 真实全量链路 + 全端点比对），日常开发、修复、重构**严禁主动运行**——默认快速套件中的秒级覆盖护栏已强制目录登记，未登记端点在普通 `mvn test` 即失败；仅两种时机运行（需 Docker 在运行，backend 目录）：① 发布打包或内网部署验收前（产出门禁）；② 用户明确要求。
- 开发服务处置（每次运行回归门禁前后必须执行）：运行前停止本机开发服务（后端 18080、前端 18181）；测试完成后必须重新拉起（`pwsh -File backend/run-backend.ps1`、`pwsh -File frontend/run-frontend.ps1`）并验证后端 `/actuator/health` 为 UP、前端 18181 可访问，不得让用户面对停摆的开发环境。
- 命令与判读：`../tools/maven/apache-maven-3.9.9/bin/mvn.cmd test -Pgolden-baseline -Dtest=GoldenBaselineChainTest`；重建快照必须显式追加 `-Dgolden.update=true`（该更新模式只写不比，重建后必须 git diff 人工审阅）。全绿 = 产出与基线零差异，可继续打包；出现差异必须停下按本节纪律处理，不得带着未解释差异继续产出。
- 增删改必须与功能变更同工作单元完成：新增端点——登记目录（护栏强制）→ 更新模式生成快照 → 用户审阅；删除端点——删除目录条目并删除对应快照文件；有意修改产出——先向用户展示差异并确认是有意变更 → 更新模式重建受影响快照 → 用户审阅；改动夹具=新基线版本——须用户确认，更新 `baseline-manifest.json`（行数与 SHA-256）并在 `docs/decisions.md` 留痕。
- 严禁：未经用户确认更新快照；用更新模式掩盖无意差异（一律视为回归，修复实现而非改快照）；手工编辑快照文件（只能由更新模式生成）；放宽掩码、调整目录分类（如挪入 EXCLUDED）或弱化比对让失败变绿；日常任务中顺手重生成快照或“整理”目录与夹具。
- 掩码二分：目录中响应 `ignore-paths` 是比对期忽略，修改后无需重跑；表快照 `ignore-columns` 是生成期剔除，修改后必须用更新模式重跑重生成。
- 涉及文件（前三项在 `backend/src/test/resources/golden-baseline/`）：`endpoint-catalog.yml` 端点目录（端点增删、用例参数、掩码、EXCLUDED+原因）；`snapshots/` 快照（仅由更新模式生成）；`fixtures/` 与 `baseline-manifest.json` 冻结夹具（改动即新基线版本）；`docs/decisions.md`（D-08）基线版本状态与变更留痕。
- 示例（新增 `GET /api/foo/stats`）：①实现接口与常规测试，跑默认套件，覆盖护栏失败并指认该端点未登记；②在 `endpoint-catalog.yml` 登记条目（`method: GET`、`category: READ`、`cases: [{id: default}]`，易变字段在 `ignorePaths` 声明，写接口参照现有 WRITE 条目补充受影响表声明）；③以 `-Dgolden.update=true` 运行生成快照 `foo/get___foo_stats__default.json`，git diff 人工检查内容合理（数据非空、字段符合契约）；④去掉更新模式复跑全绿，目录、快照与实现同单元提交。
...
- BI 的上述变化按 `docs/bi-dashboard/README.md` 路由到对应职责文件；只有真正改变平台通用能力时，才另行更新对应平台文档，并且不得在平台文档复制 BI 专属结论。
...
- 每次更新任何项目文档时，检查 `AGENTS.md` 的文件大小；若超过 32 KiB（32,768 bytes），必须在不丢失有效约束、关键事实与可执行信息的前提下，通过合并重复规则、删除失效内容和精炼表述完成压缩。
