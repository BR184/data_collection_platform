...
abstract class AbstractIssueFactRecordListService extends AbstractFactQueryService {
  ...
  protected final IssueFactRecordRepository issueFactRecordRepository;
  ...
  protected List<IssueFactRecord> applyBaseFilters(
    ...
    return rows.stream()
        ...
        .filter(view -> matchesText(view.functionName(), request.functionName()))
        ...
        .filter(view -> IssueStatusMembers.matchesSelection(view.bugStatus(), request.bugStatus()))
  ...
  protected String buildIssueLink(String sourceInstance, Long projectId, Integer issueIid) {
    return issueLinkService.issueUrl(sourceInstance, projectId, issueIid);
  }
