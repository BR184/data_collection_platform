...
create index if not exists idx_semantic_scope_entity on semantic_scope_definition(entity_type, enabled, scope_type);
create index if not exists idx_semantic_tag_group_domain_key on semantic_tag_group(domain, group_key);
create index if not exists idx_semantic_tag_value_group on semantic_tag_value(group_id, enabled, sort_order, value_key);
create index if not exists idx_semantic_tag_value_mapping_lookup on semantic_tag_value_mapping(value_id, enabled, source_instance, entity_type, source_field, match_type);
create index if not exists idx_semantic_tag_group_build_run_status on semantic_tag_group_build_run(group_id, status, started_at desc);
create index if not exists idx_segment_definition_entity_scenario on segment_definition(entity_type, scenario_key, enabled);
create index if not exists idx_segment_definition_scope on segment_definition(scope_key, enabled);
create index if not exists idx_segment_compute_run_segment_status on segment_compute_run(segment_id, status, started_at desc);
create index if not exists idx_segment_member_current_run on segment_member_current(segment_id, computed_run_id, entity_type, entity_id);
create index if not exists idx_segment_member_stage_run on segment_member_stage(computed_run_id, segment_id, entity_type, entity_id);
create index if not exists idx_segment_member_audit_segment on segment_member_audit(segment_id, entity_type, entity_id, created_at desc);
create index if not exists idx_segment_snapshot_source on segment_snapshot(source_segment_id, created_at desc);
