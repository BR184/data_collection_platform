create sequence if not exists fact_change_version_seq;
...
create table if not exists sync_run_table_states (
    ...
    last_delete_reconciled_at timestamp,
...
create table if not exists sync_run_authoritative_scopes (
    id bigserial primary key,
    run_id bigint not null references sync_runs(id) on delete cascade,
    source_instance varchar(128) not null,
...
create table if not exists fact_change_heads (
    source_instance varchar(128) not null,
...
create table if not exists sync_run_fact_targets (
    ...
    constraint ck_sync_run_fact_targets_versions
        check (change_version > 0
            and (published_version is null or published_version >= change_version))
);

create table if not exists fact_projection_generations (
    source_instance varchar(128) not null,
    fact_type varchar(64) not null,
...
create table if not exists fact_projection_refresh_tasks (
    id bigserial primary key,
...
create table if not exists sync_run_publication_fences (
    id bigserial primary key,
    run_id bigint not null references sync_runs(id) on delete cascade,
...
create table if not exists sync_run_publication_fence_scopes (
...
create table if not exists code_review_external_metrics (
    ...
    raw_payload text,
    metric_source_updated_at timestamp,
    enrichment_status varchar(32) not null default 'SUCCESS',
    enrichment_version integer not null default 1,
    enrichment_attempts integer not null default 0,
    enrichment_error varchar(512),
    enrichment_next_attempt_at timestamp,
    enrichment_attempted_at timestamp,
    enrichment_succeeded_at timestamp,
...
create table if not exists code_review_match_mode_db_settings (
    ...
    review_data_read_mode varchar(32) not null default 'compatibility',
    code_review_read_mode varchar(32) not null default 'compatibility',
    created_at timestamp not null default current_timestamp,
    updated_at timestamp not null default current_timestamp,
    ...
    constraint ck_code_review_match_mode_review_data_read_mode
        check (review_data_read_mode in ('compatibility', 'formal')),
    constraint ck_code_review_match_mode_code_review_read_mode
        check (code_review_read_mode in ('compatibility', 'formal'))
);
...
create table if not exists review_data_match_mode_edit_links (
    ...
    authority varchar(32) not null default 'PLATFORM_OWNED',
    created_at timestamp not null default current_timestamp,
    updated_at timestamp not null default current_timestamp,
    unique (match_mode_report_id),
    unique (match_mode_report_legacy_id),
    unique (review_record_id),
    constraint ck_review_match_mode_edit_links_authority
...
create table if not exists issue_scope_groups (
    ...
    sort_order integer not null default 0,
    enabled boolean not null default true,
    definition_generation bigint not null default 1,
    remark varchar(255),
    created_at timestamp not null default current_timestamp,
    updated_at timestamp not null default current_timestamp,
...
alter table sync_run_table_tasks add column if not exists lookup_scope_json text;
alter table sync_run_table_tasks drop column if exists lookup_column;
alter table sync_run_table_tasks drop column if exists lookup_value;
...
-- 兼容评审报告的 JSON ID 数组先展开为精确关系，下游读模型不解析字符串。
...
-- 评审统一读模型：兼容态只追加未由平台接管的 Mongo 快照；正式态仅保留正式表。
