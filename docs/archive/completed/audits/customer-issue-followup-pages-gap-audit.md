...
2026-06-18 按老平台源码、规则总表和新平台真实接口再查一轮后，发现本页仍有两处展示/入口差异：

- 老平台 `IssueRespEfficiency.vue` 进入页面后会读取 `getAllMileStoneName(projectId=325)`，并默认选中第一个里程碑，再以 `milestone` 调用 `/dataAnalysis/getIssueRespEfficiency`。新平台当前统一统计板路由没有挂客户问题里程碑数据范围条；如果 URL 或条件筛选没有传 `milestoneTitle`，接口会按全部客户问题范围聚合。当前本地库刚好只有一个里程碑，页面结果不暴露问题，但多里程碑数据下会与老平台默认页面集合不同。
- 老平台本页主表只有各模块行，不渲染“总计”行。新平台当前 `CustomerIssueResponseEfficiencyBoardService` 会追加 `rowKey=__total__` 的“总计”行。该行属于新平台统计板通用增强，但不是老平台本页可见字段/行集合。

已确认仍然对齐的部分：

- 主表指标列为“产品版本 / 响应周期（小时） / 解决周期（天）”。
- 响应周期只统计 `research_template_time` 非空议题，平均小时数四舍五入取整。
- 解决周期只统计 `bug_status` 包含“已修复/完成”且 `fixed_label_time` 非空议题，平均天数保留 1 位小数。
- 响应周期和解决周期单元格的下钻集合已按老平台条件收口。
...
2026-06-18 复核老平台源码和规则汇总文件时，未找到“严重程度 / 紧急程度 / 议题状态 / 测试状态 / 非法类型”等字段的明确颜色映射规则。老平台相关客户问题和系统测试列表页大多以普通文本列展示这些值；后端枚举也只保存业务值，没有保存颜色、标签类型或样式字段。

该点先标记为待业务确认：这些字段的颜色可能来自内网 GitLab 开发人员打标签时的隐含规范或后续产品化展示要求，目前规则总表尚未记录。后续若要保留或新增颜色展示，应先补充规则来源，再统一沉到共享展示层；不能在各页面零散硬编码。
