...
1. 评审数据管理的“模板”按钮。（历史判断已作废：后续复核确认老平台有该入口，新平台已恢复。）
2. `/api/review-data/template` 接口。（历史判断已作废：后续复核确认老平台有该接口能力，新平台已恢复。）
...
4. 之前关于评审数据“模板”按钮和 `/api/review-data/template` 的删除结论已作废。后续复核确认它们属于老平台已有能力，新平台已恢复模板下载入口。
...
3. 该事项不等同于评审数据管理“模板”按钮。评审数据管理模板下载已按老平台能力恢复；外部采集表单链路暂保留并等待后续完整设计和实现。
