...
新平台 `CustomerIssueIllegalRecordsView.vue` 当前主表已开始收敛，但还需最终确认：
...
上面这些已按老平台方向调整；后续只需确认没有旧字段残留出现在主表即可。

主表里不应再回到老平台没有的 `所属项目`、`优先级`、`里程碑`、`创建人`、`更新时间`。

### 2. 展开/详情字段需要补齐老平台字段并统一命名

**影响范围：中。影响用户核对非法来源。**

老平台展开区包含 `议题提交时间`、`功能名`、`测试状态` 等字段。

新平台通用详情抽屉当前已有部分字段，但仍存在命名或可见性差异：

- `创建时间` 已切换为 `议题提交时间`。
- `更新时间` 已切换为 `议题更新时间`。
- `缺陷状态` 已切换为 `测试状态`。
- `功能名` 已有条件展示，但需要确保字段来自事实层并在有值时展示。
- 详情中应明确展示 `议题处理人`，与主表字段一致。
...
新平台当前 `getFilterOptions()` 已开始合并老平台固定非法类型集合与事实层实际命中的类型，但仍需要确认数据重建后是否还能稳定显示：
...
该项已在客户问题非法页查询侧做了收口，但如果历史事实未重建，底层仍可能存在旧脏数据，仍需等事实层重建后最终确认总数。

### 6. 客户问题专属“缺陷调研模板”非法生成尚未落到事实层

**影响范围：高。直接影响非法数据总数和非法类型。**

规则总表第 5.4 和老平台代码都要求追加“未按照要求填写缺陷调研模板”。

当前新平台事实层通用 `IssueClassificationRules.illegalReasons(...)` 只包含：
...
客户问题专属调研模板完整性、计划解决时间、一级缺陷负责人签字确认规则已开始落到事实规则层，但仍需要等数据重建后确认全量命中与老平台一致。

需要补齐到事实层或客户问题专属事实构建规则中，并确保：
...
当前客户问题非法页已经按 `illegal_reasons / illegal_reason` 多值集合做了白名单筛选；后续只需关注事实重建后的最终结果，不再保留单值语义的隐患。

### 9. 导出字段已经开始补齐，但需要最终与页面核对
