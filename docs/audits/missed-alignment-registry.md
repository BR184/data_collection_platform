...
- 验证记录：前端 `npm.cmd run typecheck` 已通过；后端 `mvn -q -DskipTests compile` 首次失败于新增 `trimToNull` 与父类 protected 方法权限冲突，已改名为 `trimTextToNull`。按本仓“失败即停、不重复验证”规则，后端编译留待下一次单独复验。
...
- 当前状态：已补齐代码，待后端编译复验
- 主要缺口：页级测试阶段级联切换入口已接入；需下一轮复验后再从漏项清单关闭。
...
- 当前判断：未找到独立 gap audit。
- 后续处理：需要对照老平台质量看板入口、规则汇总和当前 `QualityBoardRdView`，确认指标、图表、默认范围和跳转是否对齐。

### 2. 质量看板 / 其他看板

- 页面入口：`quality-board-other-board`
- 当前判断：未找到独立 gap audit。
- 后续处理：需要确认老平台是否有等价页面；若没有，应标记为新平台增强能力并走保留审批。

### 3. 评审数据 / 评审数据管理

- 页面入口：`review-data-home`
- 当前判断：已有多份评审数据相关实现和测试文档，但本轮未找到像系统测试/客户问题页面那样的独立 gap audit 闭环。
- 后续处理：需要单独复核老平台评审数据列表、导入、问题项、导出、筛选和模板残留清理结果。
