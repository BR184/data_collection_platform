...
- 当前判断：已建立 `docs/audits/review-data-management-gap-audit.md`，本轮二次对齐已完成并通过冒烟与真实链路验证。
- 处理结果：直接筛选语义、默认列、导出列、模板下载入口和新增后继续录入评审问题闭环已按老平台恢复。
- 后续处理：若业务方要求恢复“按模板文件直接导入单个评审报告”，需另行对齐旧平台上传解析链路；当前迁移导入和模板下载不混用。
...
- 当前判断：该页属于新平台新增能力，标签组边界以 `docs/architecture.md` 为准，不属于老平台已有页面 1:1 重构。
