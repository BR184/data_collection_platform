...
1. **缺少老平台同等的测试阶段级联切换入口。**
   ...
   结论：这是上一轮对齐漏项。审计文档基线已记录“测试阶段级联选择”，但最终收尾时误写为“暂无已确认缺口”。后续实现应按 `docs/platform-page-business-rules.md` 的范围优先级和 `docs/architecture.md` 的统计板边界补齐，而不是把阶段切换继续混进普通高级筛选里。
