...
1. 老平台“系统测试横向对比 Excel 下载”在新平台未找到等价导出 API，本轮未硬造口径，仍作为功能可达性缺口保留。
2. 老平台展示“任务执行时间/执行时长”，新平台当前展示镜像/事实刷新状态和最近同步时间，尚未 1:1 展示这两个字段。
3. 建议类列与系统测试公共排除“建议”之间仍保持老平台现状：公共过滤会排除建议，列保留但可能为 0。后续若业务确认建议类需统计，需要先更新 `docs/platform-page-business-rules.md`。
