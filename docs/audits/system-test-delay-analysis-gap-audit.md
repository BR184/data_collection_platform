...
1. 老平台首次进入页面会默认选中阶段列表第一项；新平台统一统计板未选择测试阶段时展示全部系统测试范围。该项会影响默认首屏范围，但属于统一统计板筛选策略差异，本轮未强行改动默认筛选机制。
2. 老平台多元看板中的堆叠柱状图能力，新平台当前由统一统计表格和多看板摘要承载。本轮先确保数据字段、数量和下钻一致，不新增图表形态。
