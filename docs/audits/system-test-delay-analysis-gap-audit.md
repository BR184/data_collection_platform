...
本轮已按老平台代码和规则汇总文件完成以下对齐：

1. 延期原因行集合改为老平台 `DelayEnum` 固定 7 项：技术卡点、方案卡点、资源卡点、数据异常、算法问题、机制问题、计算效率。即使数量为 0 也保留行。
2. 主表排序改为老平台固定枚举顺序，不再按事实层出现值动态排序。
3. 主表不再额外追加新平台自定义的 `共计` 行，避免页面和导出比老平台多一行。
4. 主表列名对齐老平台页面展示：一级缺陷(个)、二级缺陷(个)、三级缺陷(个)、建议类缺陷(个)、总计(个)。
5. 建议类缺陷按老平台 `category like 建议` 口径统计，并兼容事实层已有 `severity_level = SUGGESTION`。
6. 总计按老平台独立表格口径计算为一级缺陷 + 二级缺陷 + 三级缺陷 + 建议类缺陷。
7. 下钻字段对齐老平台通用议题详情字段：议题编号、模块名、议题标题、议题状态、严重程度、测试状态、延期原因、议题更新时间、议题提交时间、议题提交人、议题处理人。
8. 下钻点击一级/二级/三级/建议类/总计时，使用与主表完全一致的匹配口径。
9. 规则说明同步更新为固定延期原因、建议类和总计口径，避免页面说明与实际统计不一致。

仍作为后续确认项保留：

1. 老平台多元看板中的堆叠柱状图能力，新平台当前由统一统计表格和多看板摘要承载。本轮先确保数据字段、数量和下钻一致，不新增图表形态。

## 2026-06-15 再复查补齐记录

本轮继续对照老平台 `DelayCauseInfo.vue`、`DataAnalysisController`、`SpiderIssueDataDAOImpl` 和 `QueryUtil` 后，补齐或澄清以下项：

1. 默认测试阶段已补齐。老平台首次进入页面会从 `getLabelPhase()` 返回列表中取第一项作为 `testingPhase` 并立即查询；新平台已在无测试阶段筛选时按阶段选项第一项生成实际筛选条件，并通过 `appliedFilterGroup` 回填到筛选栏，避免首屏查询全量系统测试范围。
2. 规则说明的阶段过滤说明已同步改为“未选择时按老平台默认使用阶段列表第一项”。
3. 导出复用当前统计板查询结果，默认测试阶段同样生效，避免导出绕过首屏口径。
4. 复查确认老平台 `QueryUtil.setQueryFilter()` 对非客户项目会排除 `category like 建议`，因此本页当前继续使用事实层 `is_excluded = false` 与老平台实际公共过滤行为一致。虽然老平台表头保留“建议类缺陷(个)”，但在该公共过滤下建议类数据可能被过滤，这是老平台代码的实际口径，不按“更合理”方式改写。
