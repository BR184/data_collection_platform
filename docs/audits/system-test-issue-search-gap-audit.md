...
1. 分页默认值和分页组件样式。
2. 新平台支持排序、标签组筛选、刷新状态、设置按钮，这些属于体验增强；只要不改变数据口径，可保留。
...
议题查询页不再保留“综合搜索”行，也不再保留只覆盖部分字段的快捷关键词搜索框。

原因是该入口语义不清：用户看到“综合搜索”会自然理解为所有字段均可匹配，但旧实现只覆盖议题编号、标题、模块、功能等少数字段，容易造成“搜不到就是没有数据”的误判。议题查询页本身已经提供更完整的字段筛选、条件筛选和标签组筛选，应以这些明确字段语义的筛选能力作为主入口。

### 已落地

1. 页面移除 `searchType` 和 `keyword` 相关筛选控件。
2. 页面请求参数不再发送 `searchType` 和 `keyword`。
3. 当前页路由白名单不再接受 `searchType` 和 `keyword`。
4. 原高级筛选字段已前置为直接可见的字段筛选，包含议题编号、标题、项目名称、提交人、处理人、议题状态、严重程度、测试状态、类别、里程碑、提交时间、更新时间、测试阶段、模块关键词和功能名。
5. 条件筛选组件继续保留，用于更复杂的字段组合、标签组筛选和后续记录类表格复用。

### 后续复核口径

后续复核议题查询页时，不应再把“缺少综合关键词搜索框”标记为缺口。需要检查的是：所有老平台字段筛选是否可达，字段候选是否准确，查询和导出是否使用同一事实层过滤语义。
