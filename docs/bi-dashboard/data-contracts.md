...
- 编码事实按依赖字段分别去重和判冲突：新增行或合并时间冲突不影响仅依赖稳定身份的合并请求计数；作者冲突不清空代码规模和模块增量；模块冲突不清空人员贡献；走查质量、问题类别、静态扫描和注释率也只受各自事实族完整性约束。
...
| 编码 | 数据采集平台 | 已发布合并请求、代码走查、静态扫描等事实与 BI 独立计算 |
...
- 首期不输出 `.xlsx`、CSV、原始明细包或整页长截图。平台现有 Excel 导出与 BI PNG 下载是不同能力，不互相复用业务实现。
