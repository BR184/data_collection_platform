...
- 当前阶段：2026-08-10，内网 BI/CAT 缺口的生产修复和本地验证已完成。CAT 配置、显式产品版本/阶段映射、受限 HTTP 客户端、定时全量、手动全量、全量补偿、原始响应留存、阶段原子发布和失败保留上一版均已实现；配置与调度状态现以单事务保存。单元/集成页面只读取 `bi_cat_*` 当前发布快照，不在页面请求中直连 CAT。真实 CAT 认证、证书、完整路径、用例计数和来源身份仍按已确认边界等待内网联调。
