...
- 当前阶段：2026-09-14，内网实测与工具演进三项结论已记录根因；其中 ② 的 BI 侧门槛已经用户批准当日修复（计划 `docs/plans/bi-intranet-two-defects-solution-20260914.md` 工作单元 B），① 仍只记录不修（用户先前指示“先按下不表”）；平台/数据侧根因见 `../progress.md` 同日条目：
  ...
  ② **代码走查缺陷密度曲线为空——BI 侧已修复（2026-09-14，覆盖率语义，`bi-coding-v4`）**：原门槛为“全有全无”——所有走查记录都满足“走查日期非空 && added_lines > 0 && defect_count >= 0”且同 MR 行数无冲突才计算，否则整条置空；而注释率不受行数影响，故出现“注释率有、密度空”。内网实测：符合基础条件 27,787 行中 added_lines=0 共 1,446 行、同 MR 行数冲突 745 个 MR，坏行每月都有→曲线恒空。现 `BiCodingCalculator` 改为与 static-scan/comment-rate 一致的覆盖率语义：坏行只剔除自身，密度趋势恒基于合法子集（去冲突+过滤）计算，quality-trend 分区用 `coverageSection` 披露“有效观测 X/Y”；子集口径额外要求稳定 MR 身份（CD-39 同 MR 去重前提）。口径已登记核对表 CD-39；`/api/bi/coding` 产出有意变更，黄金基线 bi 快照待发布门禁以更新模式重建审阅（日常纪律不跑）。归因修正（经本仓代码实证）：BI 走查 `added_lines` 直读 `code_review_formal/match_mode_records`（老平台镜像/导入），与 `ods_gitlab_merge_request_diffs` 无因果（该表只喂 `merge_request_commit_fact`），“同一件事的两面”说法不成立；数据侧根治（S2-C：回填 1,446 行、统一 745 个冲突 MR 口径）仍单独立项待裁定。
