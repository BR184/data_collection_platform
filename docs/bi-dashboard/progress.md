...
- **评审补正收口（2026-09-16）**：针对评审报告的五项问题完成修复：补正 `SystemTestStageContent.test.ts` 合法夹具；密度趋势排除冲突走查 ID，消除输入顺序导致的首条记录偏差；自适应值轴取整后继续满足最低量程；断轴遮罩改为非交互并用真实 ECharts/ZRender 命中测试验证散点悬浮详情；密度完整性与走查工时能力拆分。后端编码相关定向测试 **43/43**，前端全量 **134 个文件/530 项**，`npm run typecheck`、`npm run lint`、`npm run build` 和 Impeccable detector 均通过。后端默认 `mvn test` 为 1361 项、0 failures、76 errors、1 skipped，错误均因本机未启动 `localhost:15433` 测试 PostgreSQL，未出现业务断言失败。黄金基线更新模式和严格比对各 **191/191** 全绿，编码快照已人工审阅；门禁后端 18080 `/actuator/health` 为 `UP`、前端 18181 HTTP 200。工作树原有的其它未提交改动仍保持原状，本条只覆盖本工作单元。

- **初次审批记录（补正前，2026-09-16）**：在实现交付的普通验证基础上，本次修复了两个审批发现：可选 `commentRateSource` 不再参与注释率事实冲突签名；质量趋势分区分别报告注释率和密度的记录级覆盖率，避免把两类事实相加后误标为单一“走查记录”覆盖率。后端 `BiCodingCalculatorTest` 31 项、前端 `CodingStageContent.test.ts` 3 项、前端 ESLint 均通过；黄金基线更新模式和随后不带更新参数的 `GoldenBaselineChainTest` 各 191 项全绿。编码快照已更新为 `bi-coding-v5`，使用周期级 `commentRateTrend`，2025-11-17 注释率为 13.21；旧 `commentRatePoints` 已移除。门禁完成后已重启后端 18080（`/actuator/health` = `UP`）和前端 18181（HTTP 200）。工作树原有的其它未提交改动仍保持原状，本条只覆盖本工作单元的审批与回归结果。

> 以下“当前阶段”段落保留补正前的初次实现证据；若与本条评审补正收口结果冲突，以本条及工作单元计划的“评审补正交付记录”为准。

- 当前阶段：2026-09-16，编码页注释率趋势改为按请求粒度的后端非加权算术平均（计划 `docs/plans/bi-coding-comment-rate-period-aggregation-20260916.md`，决策 `docs/bi-dashboard/decisions.md` D-03）：修正「代码注释率与走查密度趋势」中注释率轨取值错误与两轨粒度不一致。原实现 `BiCodingCalculator.commentRatePoints` 逐条返回 `comment_rate`，前端 `CodingStageContent.vue` 以 `observedOn` 为键构造 `Map` 折叠，重复键保留最后一次写入 → 同一周期展示值取决于来源行顺序（`merged_at_source, id`）；样例 2025-11-17 有 13 条记录，展示 `0.82` 而当日算术平均为 `13.21`。密度轨 `reviewDensityTrend` 早已按 `bucket(reviewedOn, granularity)` 聚合，两轨输出层级不一致是根因。现冻结核对表 **CD-38A**：按请求粒度分桶（日=自然日，周=周一所在日期，与 CD-39 同规则），对桶内合法 `comment_rate` 做非加权算术平均、忽略 `NULL`、先累加原始值后一次性 `HALF_UP` 保留 2 位；记录粒度为 CD-16 稳定走查记录 ID，完全相同重复事实只计一次，同一 ID 核心字段冲突时整组不进入聚合且 `comment-rate`/`quality-trend` 区块标记 `INCOMPLETE`。页面 DTO 直接替换 `commentRatePoints` → `commentRateTrend(period, averageCommentRate)`，删除旧字段、不留兼容别名或双轨；`RULE_VERSION` 由 `bi-coding-v4` 升 `bi-coding-v5`。无合法记录的周期不返回点位，前端与密度轨对齐后显示 `null` 并保持断线（断点本身是诚实表达，不消除）。原「注释率同日/周聚合公式未冻结」阻塞就此解除。验证（2026-09-16 实施完成）：后端定向 `BiCodingCalculatorTest` 30 项全绿（新增同日算术平均、忽略 NULL/负值/缺日期、周桶周一起点、非加权、完全重复只计一次、冲突整组剔除且区块 INCOMPLETE、无合法桶不补零 7 项）；后端默认套件 1358 项 0 错误，仅 `SqlPushdownRealChainTest` 4 项失败（`setUp→loginHeaders` 登录前置失败，本地身份服务不接受 `admin/admin-test-2026`，与本单元无关；另需先启动 `qaflex-test-postgres-15433` 才消除 76 个 PG 连接错误）；前端 BI 定向 82 项、全量 133 文件 527 项全绿；ESLint 0 错；`tsc --noEmit` 仅报他人未提交的 `SystemTestStageContent.test.ts` 三处 `'FAILED'` 不在 `BiDataStatus` 内（HEAD 无此串，非本单元）；`vite build` 因沙箱拦截 dist 批量删除与 `src/components.d.ts` 写入未跑完（已转换 2743 模块后中断，被误写的生成文件已 `git checkout` 还原）。旧/新对照（冻结快照 2425 条逐记录）：2025-11-17 旧 0.82 → 新 13.21（13 条，min 0/max 100）；日粒度 169 周期 ⊆ 密度 176 周期，仍保留 7 天真实断点（2025-10-20、10-22、10-25、10-28、10-29、11-03、11-04）；周粒度 35 周期（2025-11-17 周平均 9.39）。黄金基线 `bi/get___coding__day-all.json` 已在本次审批后按门禁完成更新模式重建并人工审阅，随后 compare 模式复跑 191 项全绿；编码快照已更新为 `bi-coding-v5` 与 `commentRateTrend`，不再保留旧 `commentRatePoints`。

- 当前阶段：2026-09-15，编码页静态代码扫描图表下线（工作单元 SA，计划 `docs/plans/bi-static-scan-retire-and-regression-20260915.md`，决策 D-12）：用户裁定静态扫描数据太薄（仅 scan_status 三态 + scan_bug_count 计数，无严重级别/分类/规则/文件/模块维度，量纲混淆；`product.md` 旧述“阻断/严重/一般/提示堆叠”从未被数据支持），先下线，待 PMD-Biome 以结构化 `issues[]` 接入后重建。全栈移除：前端删卡片+`scanData`/`scanChart`/词条+`types.ts` 字段；后端删 `BiCodingCalculator` 的 scanFacts/scanValues/static-scan 分区/`ScanPoint`/`scanTrend`/`scanCoverage`/CD-36、CD-37 溯源、`BiCodingSource.scanDataAvailable`、适配器 `scan_status`/`scan_bug_count` SELECT、`BiDownloadAuthorizationService` 的 coding 白名单 `stacked-category-bar`；最终 `RULE_VERSION` 为 `bi-coding-v5`（静态扫描本身不另行升版，最终版本由同一变更集的 CD-38A 聚合口径统一）。布局重排为“问题分布+人员贡献+模块增量”三卡等宽（各 compact）。红线：DB 列 `code_review_*.scan_status/scan_bug_count`（代码走查记录页消费）与共享图表类 `StackedCategoryBarChart`（SystemTest 复用）保留。验证：后端定向 BiCodingCalculatorTest 23/BiPlatformSourceAdapterTest 10/BiDownloadAuthorizationServiceTest 1 全绿、前端 BI 81/81 绿。黄金基线回归（计划 B-2）已跑完：真因校正——旧记“case[88] BI coding 超时”实为 `code-review/illegal-records/export` Excel 导出超 120s（非 coding，动态测试彼此独立）；给 `GoldenBaselineSupport.getBytes` 单独加长 `EXPORT_READ_TIMEOUT`（仅二进制导出，JSON 端点 120s 不变）后更新模式 191/191 零 error。`bi/get___coding__day-all.json` 重建后逐项审阅 diff 恰为 6.4 八项预期（最终 `ruleVersion` v5、static-scan/scanTrend/scanCoverage/CD-36-37 删、reviewDensityTrend 由空变合法子集），含 scan 列的走查记录页列表快照零 diff；compare 复跑 191 例、coding 全绿，仅 2 例 statistic-boards（board v13→v14、numericValue 0→null）为他单元既有基线漂移（另册已登记），未在本单元吞入；其余 66 个因 CRLF/易变字段/键序噪声已 `git restore`。当前工作树快照区仅剩该 coding 快照待提交。
- 当前阶段：2026-09-14，内网实测与工具演进三项结论已记录根因；其中 ② 的 BI 侧门槛已经用户批准当日修复（计划 `docs/plans/bi-intranet-two-defects-solution-20260914.md` 工作单元 B），① 仍只记录不修（用户先前指示“先按下不表”）；平台/数据侧根因见 `../progress.md` 同日条目：
  ① **切换阶段页面报“coding 页面读取期间来源版本发生变化，请重新刷新整页”**（偶发）：BI 页面读取是“前后双指纹”一致性锁——`BiPlatformCodingSourceAdapter`/`BiPlatformCodeReviewSourceContextFactory` 在 `capture()` 先算一次数据指纹（dataVersion），执行编码页大 SQL（本身耗时数秒~数十秒），`verifyDataVersion()` 读取结束后再算一次指纹对比，不一致即抛 `BiSourceVersionChangedException`。指纹最关键一段是 merge_request 事实投影全局版本（`FactProjectionVersionService._globalSourceVersion("global", MERGE_REQUEST)`：对 `fact_projection_generations` 表所有 MERGE_REQUEST 行 generation 聚合取哈希，任一项目 generation 变则指纹变）。每 15 分钟一轮 INCREMENTAL_SYNC→FACT_REFRESH 持续刷新该 generation，读取恰好落在 FACT_REFRESH 写窗口内即失败，故偶发；刷新能否成功取决于刷新时 FACT_REFRESH 是否仍在写。这不是故障而是平台强一致性保护（避免呈现“读一半被改”的拼接结果），真正问题是 FACT_REFRESH 单次耗时过长把“指纹稳定窗口”压得很窄（平台侧耗时数据见 `../progress.md`）。
  ② **代码走查缺陷密度曲线为空——BI 侧已修复（2026-09-14，覆盖率语义，最终并入 `bi-coding-v5`）**：原门槛为“全有全无”——所有走查记录都满足“走查日期非空 && added_lines > 0 && defect_count >= 0”且同 MR 行数无冲突才计算，否则整条置空；而注释率不受行数影响，故出现“注释率有、密度空”。内网实测：符合基础条件 27,787 行中 added_lines=0 共 1,446 行、同 MR 行数冲突 745 个 MR，坏行每月都有→曲线恒空。现 `BiCodingCalculator` 改为与 static-scan/comment-rate 一致的覆盖率语义：坏行只剔除自身，密度趋势恒基于合法子集（去冲突+过滤）计算，quality-trend 分区用 `coverageSection` 披露“有效观测 X/Y”；子集口径额外要求稳定 MR 身份（CD-39 同 MR 去重前提）。口径已登记核对表 CD-39；`/api/bi/coding` 产出有意变更，黄金基线已按最终 v5 快照重建审阅。归因修正（经本仓代码实证）：BI 走查 `added_lines` 直读 `code_review_formal/match_mode_records`（老平台镜像/导入），与 `ods_gitlab_merge_request_diffs` 无因果（该表只喂 `merge_request_commit_fact`），“同一件事的两面”说法不成立；数据侧根治（S2-C：回填 1,446 行、统一 745 个冲突 MR 口径）仍单独立项待裁定。
  ③ **静态代码扫描图表与 SonarQube→PMD-Biome 演进**（2026-09-15 已裁定下线并执行，见 D-12 与本文顶部同日条目）：SonarQube 后续将被 PMD-Biome 替换（工具已实现但尚未完全替代，替代时间未知）。本地 `D:\projects\pmd-biome` 实证：PMD-Biome 每次扫描产出结构化 `issues[]`（tool=PMD/Biome/ESLint、severity=Error/Warning、category=bug/code-smell/security-hotspot/vulnerability、rule、文件行列、中文 message、GitLab 深链）与 totalCount/warningCount/errorCount，规则约 346 条（PMD 221 + Biome Grit 107 + ESLint 18）均带严重级别与分类；交付通道为 HTTP API `/api/v1/tasks/<id>/artifacts/issues.json`、文件系统 `data/<job>/#<build>/reports/issues.json`、以及仅写 Markdown 摘要的共享 SQLite `external_tool_comments`（对 BI 太薄）。当前平台全库对 pmd_biome/external_tool_comments/onepanel **零引用**，即 PMD-Biome 数据尚未接入；原静态扫描图仅走老平台 `sonar_qube_result`+`bug_count`（仅 3 个状态串+未关闭问题数，无分级/分类/规则/文件维度，量纲混淆无法挽救）。裁定：已按 D-12 下线现有静态扫描图（工作单元 SA）；待 PMD-Biome 完全替代并接入平台后用真实分级数据重建（严重级别/分类/工具/Top 规则/模块密度趋势等维度）；PMD-Biome 接入立项为独立工作单元待领导裁定。
- 当前阶段：2026-09-14，重尾值轴可读性修复（计划 `docs/plans/bi-chart-adaptive-axis-tail-break-20260914.md`，纯前端图表层）：编码页“代码注释率与走查密度趋势”密度轨与“单次人工走查质量分布”散点因个别极端值（密度 100+/速率数百）导致主体（多为个位数）被压扁。唯一轴工具 `adaptive-value-axis.ts` 从“相邻观测显著空白间隔才断轴”（对连续重尾失效，主体上界被中档离群值顶高）重写为“百分位主体+单段尾部断轴”：主体上界取 min(95 分位, 4×中位数) 且≥minimumMax，步长取对数距离最近的 nice 值（1/2/5/10 梯，不向上取整避免撑大近两倍）；极大值超主体×1.25 时整个尾部压入单段原生 `AxisBreak` 锯齿带，带顶标签=真实极大值，带内极值仍绘制、tooltip/Excel/PNG 保留真值；无尾部退化为连续均匀轴。消费方 6 条值轴（趋势双轨、散点 X/Y、双面板 X/X）自动受益，图表类零逻辑改动。另答疑：注释率折线断开非缺陷，是“当日无合法注释率观测”的诚实表达（前端周期取两序列并集 + `connectNulls:false`；内网注释率覆盖 2425/2634、密度覆盖 2585/2634，断点分布与覆盖率吻合），不改。词条（三散点+质量趋势）已补断轴/断点含义。验证：BI 图表定向 41/41、前端全量 526/526、eslint 0 错、三项仓库门禁 exit 0（一次并发高压运行出现 1 项与本单元无关的计时敏感 mount 冒烟抖动，隔离复跑稳定通过）。浏览器真实界面验收待用户确认。
...
- 当前阶段：2026-09-10，已完成 BI 看板实测反馈修复与全看板图表业务说明落地（v1.0 十项修复 + v2.0 复核修正），计划 `docs/plans/bi-dashboard-feedback-and-fixes-comprehensive-20260910.md`：
  ...
  ⑪ 系统测试各模块修复率达成情况表头文字列对齐（BUG-07）：抽取 `computeModuleRepairColumnLayout` 权威列几何函数，锁定 `grid.containLabel: false`，表头 `graphic` 采用 `x: col.center` 配合 `textAlign: 'center'` 绝对居中对齐，已获用户确认满意；
  ⑫ 缺陷严重度分布图表一级占比对齐与防重叠（BUG-08）：查清根本原因为 `labelLayout: { moveOverlap: 'shiftY' }` 误伤左上方高位文本导致其下移 29.8px 与折线末端脱节，移除该后置配置后文本恢复精准对接在折线末端（Y=23.6px，折线对准两行文字正中），圆环饱满尺寸（112px）与两行格式完全保留，重叠彻底消除；
  ⑬ 系统测试最上方 7 指标条左右组次序调整（BUG-09）：按领导最新要求，将后方 4 项缺陷概览（累计发现数、已修复数、当前未修复数、整体修复率）整体移动至最前面，前 3 项质量目标修复率（一级缺陷、P1、P2）整体移动至后面，各组内部相对顺序严格保持不变；增加前端单测断言并热更新生效。
...
- 当前阻塞：本地无法读取内网老平台 MySQL 和平台 PostgreSQL，因此 `spider_crowncad_data` 的扫描/注释率覆盖、设计正分母覆盖及 3583 条对应的具体内网运行 ID 需在内网执行计划中的只读 SQL；控制流根因本身已经闭环。静态扫描四级严重度的物理字段与业务公式没有已确认来源，实施前需冻结目标契约（注释率同日/周聚合公式已于 2026-09-16 冻结为核对表 CD-38A，见本文件顶部同日条目）。CAT 手册仍缺基地址完整路径、认证、可信证书、执行/通过用例数、原子来源身份和稳定分页契约；当前主机仅证明 88 端口 TCP 可达，TLS/HTTP 未在 3 秒内完成，不能据此推断内网后端容器的最终联通性。运行产物位置门禁仍仅被仓库根目录既有 8 个 `.tmp-*.log` 阻塞，本工作单元未创建、删除或移动这些用户文件。
