...
### 2026-06-25 第三轮：一级分类口径与非法规则收口

已完成：

1. 系统测试一级缺陷“其他”口径收回老平台规则：标题只要命中 `退`、`回退`、`倒退`、`挂机`，就不计入其他一级。
2. 系统测试非法数据移除“流程越位”这类新平台扩展非法类型，非法口径回到老平台已确认的四类：未设定严重程度、未设定模块、未按照模板回复、缺陷原因不唯一。
3. 相关单测已同步更新，避免后续回归把这两个口径再扩宽。

影响项：

- 问题 3：系统测试缺陷汇总/平台/其他 多 1 条。
- 问题 8：系统测试非法数据量偏大。

当前仍待真实环境复核：

- 问题 4、5、9、11 仍需要用内网真实数据和老平台导出明细继续对表，暂不做无证据扩展。

### 2026-06-25 第四轮：剩余默认范围差异收口

已完成：

1. 系统测试议题查询默认项目收回老平台 CrownCAD 口径。列表、导出和筛选候选在未显式传 `projectId` 时统一使用 CrownCAD 项目 `9`，避免默认全项目导致数量比老平台多约 1W。
2. 代码走查非法数据默认分支过滤收回老平台行为。老平台只在明确选择 `CrownCAD` 或 `DGM` 且目标分支为空时补 `dev`；全项目非法数据查询不应因为 `sourceInstance=cc/default` 被隐式限制到 `dev`。新平台已移除全项目默认 `dev` 过滤，保留用户显式目标分支筛选。
3. 已复核老平台严重程度和模块标签解析：严重程度来自 `严重程度` 标签值，合法值为 `一级缺陷`、`二级缺陷`、`三级缺陷`；模块来自 `模块` 和 `工具箱` 标签。当前没有源码证据支持为“装配/三级缺陷少 1”新增标题、裸标签或单条特殊规则。

影响项：

- 问题 9：系统测试/议题查询数据量比老平台多约 1W。
- 问题 11：代码走查非法数据页面比老平台少 6000 条。
