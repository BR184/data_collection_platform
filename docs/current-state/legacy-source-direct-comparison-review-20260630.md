...
3. **P0-2：客户问题缺陷汇总首屏 SQL 化**
   ...
   - 当前落地状态：已先完成 `project_id=325`、`created_at_source>=2026-01-01`、`milestone_title` 和 `is_excluded=false` 的 SQL 过滤下推；模块分桶和指标计数仍保留 Java 聚合。若内网清空快照后首次打开仍超过约 3 秒，再继续将模块维度 `GROUP BY` 下推到 SQL。
4. **P0-3：客户问题缺陷原因分析首屏 SQL 化**
   - 按 `module_names + reason_category` 聚合当前客户问题 scope、当前里程碑、公共排除后的数据。
   - 保留老平台原因映射规则，不因 SQL 优化改变口径。
   - 当前落地状态：已先完成客户问题 scope、当前里程碑和公共排除的 SQL 过滤下推；原因分组和模块展开仍由 Java 按老平台原因目录组装。若内网首次缺快照路径仍慢，再评估将 `module_names + reason_category` 聚合改为 SQL `GROUP BY`。
5. **P0-4：默认快照预热**
   - 保留现有快照命中读取。
   - 事实层重建后预热客户问题 P0 页面。
...
- 当前 P0 验收重点是确认 SQL 过滤下推后缺快照路径是否可接受；若单里程碑数据量仍导致首次请求慢，再进入 SQL `GROUP BY` 下推，不在本轮继续扩大生产改动面。
