...
8. 同一主机上运行的多个平台实例必须使用彼此独立的 Session 和 CSRF Cookie 名称；Cookie 隔离不能依赖端口或 LDAP 用户名，因为浏览器 Cookie 作用域不包含端口。
9. 标准 Compose 部署将唯一且跨更新保持不变的 `COMPOSE_PROJECT_NAME` 注入为 `PLATFORM_INSTANCE_ID`。平台从该身份派生 Cookie 名称，不把部署名称直接暴露在 Cookie 中；直接运行未显式设置实例身份时使用应用名与监听端口。
10. CSRF Token 仍由平台 Session CSRF 机制校验。后端通过 `X-XSRF-TOKEN` 响应头提供当前实例 Token，前端按 Origin 保存并在非安全请求中通过同名请求头提交；LDAP 认证、Session 生命周期和本地授权语义不变。
...
8. 多实例浏览器会话隔离属于平台认证边界；实例身份配置、Cookie 命名空间和 CSRF 响应头契约必须通过部署和安全回归测试验证。
