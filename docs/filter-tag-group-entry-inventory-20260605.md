...
| `frontend/src/views/ReviewDataManagementView.vue` | Mixed: `BaseRecordTable` plus default `filter-builder` slot using `StatisticFilterBuilder`. | `<StatisticFilterBuilder :model-value="filterDraft" ... />`. | 已挂标签组（默认展开）+ 高级条件构建器折叠区 + 自动恢复快照 + Flyway seed migration. | Avoid exposing raw dirty module values as the main module entry. |
