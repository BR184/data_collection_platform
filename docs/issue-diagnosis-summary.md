...
中间汇总表/统计快照方案是正确方向，但它不能替代规则对齐和查询范围收口。推荐分三步：
...
3. 第三轮：为系统测试缺陷原因分析、议题阶段统计、申请延期缺陷分析、系统测试缺陷汇总和客户问题统计页增加统计快照表。快照键必须包含页面 key、项目/里程碑/测试阶段、模块、指标和规则版本；下钻明细必须复用同一范围条件，确保父表数字和详情总数一致。

本轮不先落统计快照表，是因为当前还有筛选范围和老平台规则差异。若先缓存，会把错误口径固化为快速错误结果；应先确保源数据范围和默认筛选对齐，再把稳定口径快照化。

## 2026-06-29 三轮内性能落地记录

本轮按“先解决会导致 15 秒超时和同步全量偏移的结构问题，再做最小真实链路验证”的顺序推进。统计快照/中间表仍是后续可选方案，但在口径仍需对齐时不先把结果固化。
...
4. 系统测试三个慢页首屏 SQL 聚合：`系统测试/议题阶段统计`、`系统测试/缺陷原因分析`、`系统测试/申请延期缺陷分析` 的主表接口不再先把全部明细拉到 JVM 聚合，而是在数据库侧按阶段、模块/原因、延期原因聚合后返回汇总行。详情、规则说明和导出暂保留原明细路径，后续若仍慢再继续 SQL 分页和流式导出。
...
3. 本轮没有引入统计快照表，因此无需事实层快照重建；但对象化事实刷新和查询口径改变仍要求内网重新跑同步/事实刷新后再验收统计数量。
...
4. 系统测试缺陷原因分析、议题阶段统计和申请延期缺陷分析的首屏主接口已在 2026-06-29 改为 SQL 聚合，不再把阶段范围内的全部 `issue_fact` 明细拉到 JVM 后聚合。仍需内网全量复测确认三张表是否稳定低于 15 秒；详情、规则说明和导出路径仍可能走原明细查询，若复测仍慢，再继续做 SQL 分页、流式导出或统计快照。
