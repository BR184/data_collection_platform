...
6. 系统测试横向对比导出已经纳入正式导出对齐范围，不再只对齐普通表格导出。当前目标模板以本轮内网导出的 `CC2026R3-系统测试数据横向对比数据.xlsx` 为准，保留评审类别占比、评审工作量/效率/速率、代码走查类别占比、代码走查工作量/效率/速率、系统测试缺陷密度等新字段；导出 sheet 为“系统测试数据分析”，文件名沿用“测试阶段-系统测试数据横向对比数据.xlsx”。修复方向为：多级表头、完整列顺序、无效行过滤、CrownCAD/DGM 分块、系统测试公共过滤、代码走查 `target_branch = dev` + `MERGED`、`CC2025R1 -> CC2025R1&R2` 评审项目映射全部复用同一专用服务，小数类字段统一保留两位。

### 全页面导出对齐要求

所有页面的所有导出入口都必须纳入老平台/当前模板对齐，不只包含主表普通导出，也包括横向对比、明细导出、下钻导出、模块功能导出、非法数据导出和页面上的快捷导出按钮。每个导出入口需要逐项确认：入口按钮、请求参数、默认项目/阶段/里程碑/模块、筛选条件映射、数据来源表、排除规则、字段顺序、字段名称、sheet 名、文件名、空值规则、数值格式、合并表头和行过滤规则。导出不能绕过页面口径，也不能使用与页面不同的事实层或原始表扫描逻辑。
