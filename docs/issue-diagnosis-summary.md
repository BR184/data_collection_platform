...
1. `系统测试/议题阶段统计`、`系统测试/缺陷原因分析`、`申请延期缺陷分析` 的主接口已落地统计快照中间表。首次缺快照时仍由页面服务按当前老平台口径生成一次，后续同一事实版本、规则版本和阶段范围直接读取 `statistic_board_snapshots`，避免每次打开页面都重复扫描大范围 `issue_fact`。
...
3. 慢加载页面按优先级进入统计快照：本轮先覆盖三个系统测试超时页，下一轮继续处理客户问题延期、缺陷原因、按功能、响应效率等接近 15 秒的页面，以及系统测试缺陷汇总是否需要同样快照化。
...
3. 第三轮：为系统测试缺陷原因分析、议题阶段统计、申请延期缺陷分析先落统计快照表。快照键包含页面 key、项目/测试阶段、规则版本、事实版本和筛选 hash；事实刷新成功后统一预热默认阶段快照，页面缺失快照时可自愈生成。系统测试缺陷汇总和客户问题统计页仍是后续快照化对象；下钻明细必须继续复用同一范围条件，确保父表数字和详情总数一致。

本轮快照只覆盖已经收口到老平台阶段范围的三个系统测试慢页，避免把仍有规则差异的页面结果提前固化为“快速但错误”的缓存。

## 2026-06-29 三轮内性能落地记录

本轮按“先解决会导致 15 秒超时和同步全量偏移的结构问题，再把已收口口径的慢页快照化”的顺序推进。统计快照/中间表不再是可选方案，已作为系统测试慢统计页的长期结构落地；尚未完成规则对齐的页面暂不缓存，避免固化错误口径。
...
4. 系统测试三个慢页首屏 SQL 聚合和统计快照：`系统测试/议题阶段统计`、`系统测试/缺陷原因分析`、`系统测试/申请延期缺陷分析` 的主表接口不再每次把全部明细拉到 JVM 聚合；页面服务按数据库侧聚合生成 `StatisticBoardResponse` 后写入 `statistic_board_snapshots`。同一 `board_key + scope_key + rule_version + source_version + filter_hash` 后续直接读快照。
5. 快照刷新闭环：`FactRefreshTaskWorkerService` 在 issue 事实刷新成功并完成任务落库后，调用 `StatisticBoardSnapshotRefreshService` 统一预热注册的 `StatisticBoardSnapshotRefresher`。当前注册页面只响应 `ISSUE` 事实类型，MR 事实刷新不会误触发系统测试快照重建。

本轮修复边界：

1. 首屏加载速度应明显改善，但下钻详情、规则说明和导出仍可能在内网全量数据下慢，需要复测后决定是否进入下一轮。
2. 客户问题缺陷原因、延期、响应效率、按功能展示等页面仍属于“事实查询后 Java 聚合”结构，已在前文列为同类风险；如果内网仍接近 15 秒，需要继续把 `project_id=325`、`created_at>=2026-01-01`、默认里程碑和聚合前推 SQL。
3. 本轮已引入统计快照表，因此内网升级后需要执行数据库迁移，并在 issue 事实刷新成功后预热快照。若升级后首次打开页面时快照为空，页面会同步生成一次快照；后续同一事实版本会直接读取中间表。
...
4. 系统测试缺陷原因分析、议题阶段统计和申请延期缺陷分析的首屏主接口已在 2026-06-29 改为 SQL 聚合并接入 `statistic_board_snapshots`。仍需内网全量复测确认三张表是否稳定低于 15 秒；详情、规则说明和导出路径仍可能走原明细查询，若复测仍慢，再继续做 SQL 分页或流式导出。
