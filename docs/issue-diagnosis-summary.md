...
3. `CC_PRODUCT议题` / 客户问题延期记录页 `CustomerIssueRecordsView.vue` 已等待里程碑候选加载并写入第一可用 `milestoneTitle` 后再加载列表，避免首屏按“全部里程碑”扫全量客户问题。
4. 代码走查非法数据页和代码走查多元看板此前存在真实问题：`source` 数据源候选异步加载，`useDataScope` 再写入默认数据源，但页面加载函数已经先按空 `source` 发起请求，形成一次“全部/空数据源”慢请求和一次默认数据源请求。本轮已给 `useDataScope` 增加 `defaultReady / ensureDefaultApplied`，并在两个代码走查页面等待默认数据源落路由后再请求业务数据。
