...
| `docs/current-state/legacy-source-direct-comparison-review-20260630.md` | 当前取证审查稿 / 待审查 | 根据 2026-06-30 用户要求，直接从老平台源码提取客户问题、系统测试和代码走查相关老 hash 路由、前端默认值、后端查询链路，并与新平台当前实现做源码级差异对比；只作为审查材料，不表示已修复。 |
...
| `docs/plans/2026-07-10-module-source-alignment-design.md` | 当前设计 / 实施中 | 固定评审、议题、代码走查和集成测试模块来源及解析边界，禁止全局模块候选污染。 |
| `docs/plans/2026-07-10-module-source-alignment.md` | 当前实施计划 | 按业务域修复模块候选、标签解析、评审原值保存和兼容模式读源隔离。 |
