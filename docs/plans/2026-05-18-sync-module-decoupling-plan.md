...
- 2026-05-18：真实链路测试补充，使用当前源码 jar 临时拉起 `dcp-current-backend:18083`，接入 `dcp-local-test_default` 平台库网络与 `gitlab-data_default` 本地 GitLab 网络；前端使用 Vite 代理当前后端，不再使用旧 `dcp-local-test-backend-1` 镜像作为结论来源。
  ...
  - System Hook 事件状态修复：已确认 `gitlab_system_hook_events.processed=false` 的根因为成功派发后只回写 `gitlab_hook_events.processed_at`，没有回写 System Hook 原始事件表；已改为成功派发或合并去重后将原始事件置为 `processed=true`，失败路径保留未处理状态用于排查。
  - System Hook 修复验收：重新打包当前源码 jar 并重启 `dcp-current-backend:18083` 后，再次执行 `scripts\gitlab-direct-sync-check.ps1 -BaseUrl http://localhost:18083 -ConfigId 2 -SystemHookSecret sync-e2e-secret-20260514 -SimulateSystemHook -PollAfterSubmission` 通过；数据库最新事件 `gitlab_system_hook_events.id=4` 为 `processed=true`，对应 `sync_runs.id=138` 为 `SYSTEM_HOOK/SUCCESS`，4 个 `PRECISE` 表任务 `issues`、`issue_assignees`、`issue_metrics`、`label_links` 均为 `SUCCESS`。
  - 验证命令：`tools\maven\apache-maven-3.9.9\bin\mvn.cmd -f backend\pom.xml test "-Dtest=GitlabSystemHookServiceTest,GitlabSystemHookAsyncDispatchServiceTest,GitlabMirrorSyncServiceTest,SourceConnectionTesterTest"`；结果 14 个测试通过。
