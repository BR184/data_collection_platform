...
本阶段先做 Excel 导入，不兼容 CSV。优先目标是导入旧平台页面“导出评审列表”生成的 `.xlsx` 汇总文件；旧平台模板下载是 `.xls`，它对应的是“单个评审文件导入模板”，字段更细但格式不同，当前版本不支持直接导入。若后续需要支持模板 `.xls`，应新增独立“模板文件导入”模式，不与列表 `.xlsx` 解析器混用。
