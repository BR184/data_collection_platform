...
- `applicable_scope`：适用页面或适用字段族，例如全部兼容页面、议题页面、评审页面、人员字段族、模块字段族。
- `entity_type`、`scenario_key`、`scope_key`：可为空或为多值，用于限制业务标签组适用范围。
...
```text
系统设置 -> 标签组管理 -> 新建/编辑标签组 -> 命名和选择可见范围 -> 选择适用页面或字段族 -> 选择一个或多个语义标签类型和值 -> 校验字段兼容性 -> 保存 DSL 与 schemaHash -> 各业务表格页应用
```
...
动态语义标签类型页面展示“由系统生成”，管理员只能配置启用、排序、兜底值和说明；静态语义标签类型页面展示“由平台规则维护”，管理员可调整展示和禁用状态，但改变业务含义必须先走规则文档和测试变更。
...
- 新增 `semantic-tag-group-management` 页面：路径 `/system-settings/semantic-tag-groups`，展示语义标签类型目录及其选项。ADMIN 可维护展示属性、启用状态、排序、说明和动态刷新；业务含义只读。该页面不得让普通用户创建“专属标签组”或保存跨类型筛选组合。
- 新增 `business-tag-group-management` 页面：路径 `/system-settings/business-tag-groups`，负责业务标签组创建、编辑、禁用、可见范围、适用页面/字段族和条件维护。第一阶段所有标签组创建和修改都集中在该页面。
...
- 新增 `POST /api/business-tag-groups`，在系统设置页创建业务标签组，写入名称、可见范围、适用范围、`dsl_json`、`dsl_hash`、`tag_schema_hash` 和保存时水位。
...
- 业务标签组保存的是受控 DSL、`dsl_hash`、`tag_schema_hash`、适用范围和可见范围；加载时若 schema 不兼容，必须提示用户复核。
...
- 分群列表页加载时间小于 2 秒。
- 用户能在系统设置的业务标签组管理页创建、修改、禁用一个标签组，并设置可见范围和适用页面/字段族。
