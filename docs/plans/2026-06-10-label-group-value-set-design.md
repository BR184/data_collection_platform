...
动态规则模板和规则参数是系统内部协议，不应直接暴露给用户填写。用户应通过自然语言模板和表单化参数配置动态组，例如：
...
`ruleTemplateKey`、`ruleParamsJson`、`outputValueType` 都属于实现细节。设置页不能要求用户学习模板 key、JSON 参数名或数据库字段名。
...
1. 用户从自然语言规则模板中选择规则，例如“最近 N 天活跃处理人”“当前版本延期处理人”。
2. 模板参数使用对应控件填写，例如数字输入、日期范围、项目/来源下拉、字段选择下拉，不展示 JSON 文本框。
3. 模板应展示业务化说明和输出值说明，例如“输出：处理人字符串列表”。
4. 系统根据模板元信息或首次计算结果推断 `valueType`，页面只展示推断结果，不让用户编辑。
5. 保存前应支持预览计算结果、成员数量和失败原因。
6. 内部 `ruleTemplateKey` 和 `ruleParamsJson` 只在 API、数据库和调试日志中出现，不作为用户需要理解的字段。
...
动态规则模板需要单独登记模板元信息，而不是复用字段矩阵：

- 模板 key：系统内部唯一标识。
- 模板名称：用户可见的自然语言名称。
- 输出值类型：系统可推断的 `valueType`。
- 参数 schema：每个参数的控件类型、默认值、可选项来源和校验规则。
- 业务说明：该动态组会从哪些数据范围计算哪些值。
...
13. 动态组设置页不得暴露 `ruleTemplateKey` 和 `ruleParamsJson` 文本输入，改为自然语言模板选择和表单化参数控件。
...
18. 创建动态标签组时，用户只看到自然语言规则模板和参数控件，看不到模板 key 或 JSON 参数文本框。
