...
页面默认标签组不在设置页里单独展示“默认应用”表。系统默认标签组就是普通标签组列表中的一条记录，其名称代表作用页面，例如 `系统测试缺陷汇总`；列表中只增加“系统默认”标识。系统默认标签组由迁移预置为空的字符串静态组，用户编辑成员、启用/停用、备注时，直接编辑这条标签组。服务端可以按约定名称、页面 key、字段 key 和默认关系识别它的应用位置，但用户界面不再维护第二套默认应用配置。
