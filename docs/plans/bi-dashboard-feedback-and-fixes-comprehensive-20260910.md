...
| **14** | **BUG-07** | 系统测试各模块修复率表头文字与数据列未对齐（向右偏） | 纯前端视觉 | **已修复并获用户满意认可** | 权威列几何函数 `computeModuleRepairColumnLayout`，锁定 `containLabel: false`，表头 `x: center` 像素级中心绝对垂直对齐 |
| **15** | **BUG-08** | 缺陷严重度分布图表一级占比文字偏离折线末端并与圆环重叠 | 纯前端视觉 | **已修复并验证** | 查清根因为通用配置 `labelLayout: { moveOverlap: 'shiftY' }` 误将一级标签强行下推 29.8px 与折线末端脱节；移除该配置后文本精准对接折线末端（Y=23.6px），圆环饱满尺寸 112px 与两行格式完全保留，重叠彻底消除 |
| **16** | **BUG-09** | 系统测试最上方 7 个指标卡片左右组次序调整 | 布局与视觉 | **已修复并验证** | 后方 4 项缺陷概览（累计/已修复/未修复/修复率）整体前移，前 3 项质量目标（一级/P1/P2）整体后移，组内相对顺序严格不变 |
...
### 3.11 【BUG-07】系统测试各模块修复率达成情况图表表头文字与数据列像素级对齐修复
* **问题现象**：如图 1 所示，“整体修复率 目标95%”、“遗留缺陷未修复”、“一级缺陷 目标100%”、“P1优先级 目标90%”、“P2优先级 目标80%”等表头文字整体向右偏移（约 20px~35px），没有与其下方对应的数据列（进度条、遗留徽章、3 个优先级达标方格）垂直居中对齐，视觉明显向右歪斜。
* **技术根因**：
  1. 表头文本是由 ECharts 的 `graphic` 独立绘制的，原实现中硬编码了相对整个容器宽度的静态百分比（`left: '32%'`、`'52%'`、`'67%'`、`'79%'`、`'91%'`）；
  2. 原图表网格配置了 `grid: { left: 118, containLabel: true }`，ECharts 布局引擎在 `containLabel: true` 时会将 yAxis 文本宽度（约 104px）额外加在 `left` 外部，导致下方数据 series 实际绘制坐标系 `coordSys.x` 被推移到了 ~234px 处，而 `graphic` 无法感知这一内部动态位移；
  3. 更关键的是，在 ECharts `graphic` text 中，配置 `left` 属性是将文本包围盒的**左侧边缘**对齐到该坐标，即便设置 `textAlign: 'center'`，也只是在包围盒内部居中，导致整块多行文本被强制向右偏移了半个文本宽度。
* **修复方案（纯前端，已获用户满意认可）**：
  1. 在 `ModuleRepairMatrixChart.ts` 中抽取列几何布局权威函数 `computeModuleRepairColumnLayout`，明确定义网格起始 `startX = 118` 与右侧滚动留白，锁定 `grid.containLabel: false`，消除文字测量导致的坐标系浮动；
  2. 下方 5 列（整体进度条、遗留胶囊、一级缺陷、P1、P2）的物理中心坐标由函数统一计算（`col1Center` 〜 `col5Center`）；
  3. `graphic` 表头文本改用 `x: col.center` 配合 `style: { textAlign: 'center', textVerticalAlign: 'top' }`，由 ZRender 底层直接将文本锚点定位于各列像素级物理中心，实现像素级绝对垂直居中对齐。
* **改动文件**：`ModuleRepairMatrixChart.ts`、`chart-contract.test.ts`

### 3.12 【BUG-08】缺陷严重度分布图表一级占比文字偏离折线末端并与圆环重叠修复
* **问题现象**：在系统测试阶段 272px 卡片中，缺陷严重度分布环形图左上角红色扇区对应的“一级 占比 20.2%”文字位置整体偏离折线末端，落在折线下方，且第二行“占比 20.2%”与红色饼图边缘重叠；而二级与三级指示线对接完全正常。
* **技术根因（底层坐标实测确证）**：
  1. 三级（右上 Y=19.4px）与二级（右下 Y=254px）折线末端 Y 坐标与文字物理中心完全一致（0 像素偏差，完美居中对接）；
  2. 唯独“一级”（左上）折线末端计算在 **Y = 23.6px**，但文字中心却被推移到了 **Y = 53.4px**，向下偏离了整整 **29.8 像素**！
  3. 偏移根因是 series 配置了通用全局后置排版 `labelLayout: { moveOverlap: 'shiftY' }`：在检测到“一级”两行文字顶部接近画布上边缘时，`shiftY` 强行将文字包围盒单向沿 Y 轴向下压了近 30 像素；但 ECharts 饼图引导折线已在前置阶段固定在 Y = 23.6px，后置位移导致折线与文字脱节断裂，文字落入下方并在 Y = 53.4px 处撞上红色扇区外缘（Y = 47.8px）。
* **修复方案（纯前端，零侵入格式与尺寸）**：
  1. 移除 `DistributionDonutChart.ts` 中针对饼图不适用的 `labelLayout: { moveOverlap: 'shiftY', hideOverlap: false }`；
  2. 依赖饼图原生 `avoidLabelOverlap: true` 角度避让，无需全局 Y 轴强推；
  3. 文本中心瞬间回归至折线末端物理锚点 `Y = 23.6px`，折线精准指向“一级”与“占比 20.2%”正中，与二级、三级保持 100% 对齐风格一致；在 Y = 23.6px 下，距离红色圆环外沿拥有 15.7px 安全呼吸空隙，彻底消除重叠，圆环外径保持 112px 饱满度不变。
* **改动文件**：`DistributionDonutChart.ts`、`chart-contract.test.ts`

### 3.13 【BUG-09】系统测试最上方 7 个指标卡片左右组次序调整
* **问题现象**：原界面顶部单行指标条中，左侧展示 3 个质量目标修复率，右侧展示 4 个缺陷概览数据。领导要求将缺陷概览置于前方优先查看，达标率指标移至后方。
* **修复方案（纯前端）**：
  1. 在 `SystemTestStageContent.vue` 的模板结构中，将右侧 `bi-metric-group--overview`（4 项指标：累计发现缺陷数、已修复缺陷数、当前未修复数、整体修复率）整体移动至最左侧；
  2. 垂直细分割线 `bi-metric-bar-divider` 居中保留；
  3. 将左侧 `bi-metric-group--targets`（3 项指标：一级缺陷修复率、P1修复率、P2修复率）整体移动至右侧；
  4. 两组内部的各项顺序均严格保持不变；
  5. 在 `SystemTestStageContent.test.ts` 中补充 DOM 节点渲染顺序断言，保障顺序长期稳定受控。
* **改动文件**：`SystemTestStageContent.vue`、`SystemTestStageContent.test.ts`
