...
- 状态：阶段 5 进行中。修复已提交推送为 `553f7a49`，最终代码状态的完整工程门禁已通过。首个候选包审计发现通用 README 与本次 ODS 恢复要求冲突，已停止交付和演练；当前为打包器增加结构化恢复性 GitLab 全量同步声明后重新打包。
...
- 当前进行点：验证并提交发布契约修正，以当前现场镜像为直接基线重新生成、审计和演练更新包。
...
- 已选在发布清单、包内 README 和升级输出中统一声明恢复性 GitLab 全量同步；否决仅在包外文档追加相反指令，因为现场操作入口会产生歧义。
...
- 打包器新增 `--require-gitlab-full-sync`，对应清单 `sourceSync.fullSyncRequired=true`；它与 `--require-fact-rebuild` 互斥，并驱动包内 README 与升级完成提示生成同一恢复要求。
