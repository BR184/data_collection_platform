...
- 状态：生产实现、本地验证、提交和 GitHub 推送均已完成，对应 `main@3a3a4ff3`。打包继续暂停，未运行新的打包进程；下一步是在新内网包中按真实约 280 万总表规模验收一次性索引建设耗时和后续 10 分钟自动增量。
...
- 当前进行点：等待恢复打包并执行内网验收；不进行本地小数据容量外推。

## 恢复线索

- 当前阶段：实现、测试、构建、提交与推送已完成，等待内网发布验收。
- 恢复后首条命令：按 `deploy/intranet-offline-packaging-standard.md` 核对目标部署类型、端口、Compose project 和保数据边界后再恢复打包。
- 对应提交：`3a3a4ff3 fix(sync): 修复维表自动同步失败与反查性能`，已推送至 `origin/main`。
