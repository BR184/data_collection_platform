...
- 当前进度（2026-08-21 持续更新）：**已迁移 4/5**——DefectSummary ✅35fae152、DefectCause ✅308fd5e2（两段式 milestoneGroup/conditionGroup 模式）、ResponseEfficiency ✅db4b1f46、DelayIssues ✅63f46044（引入引擎 multiValueWithCondition 条件感知取值器，priorityCandidate 随操作符映射）。**剩余 3 板**：CustomerIssueByFunction(3方法)、MirrorTableOverview(5)、SystemTestDefectSummary(14，最大）。金标基类 AbstractStatisticBoardGoldenMasterTest 已抽取（新板金标继承它+@Test 桥接 runGoldenMaster）。每板固定闭环：金标两连跑→迁移删家族→statistics 包全量回归→独立提交推送。已知坑：板定义操作符白名单会抛 IllegalArgumentException（矩阵只能用白名单组合）；部分板无 exportIssueRecordsWorkbook（extraGoldenChecks 用 exportBoardWorkbook）；迁移后注意补 java.util.Objects import；校验层会丢弃空值条件并拒绝未知字段/操作符（引擎的未知字段恒真与 null 值分支实际不可达）。待办：SQL↔Engine 平价契约测试、基类实时状态三兄弟模板化、清理验证、architecture.md/decisions.md 同步。
