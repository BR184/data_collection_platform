...
- 收尾状态（2026-08-21 最终）：**四项收尾任务全部完成**。④SQL↔Engine 平价契约测试 ✅ `FilterEngineSqlParityTest`（@SpringBootTest 连真实开发库 15432，运行需 env TEST_DATASOURCE_URL/USERNAME/PASSWORD=qaflex 凭据；3 用例覆盖 text eq/ne/isEmpty、bugStatus plain+LABEL_GROUP、priorityLevel ne、createdAt between、OR 组合）。
  **平价测试已兑现价值——抓到两个真实双轨分歧并记录**：(a) moduleName 的 LABEL_GROUP 条件在 SQL 侧被静默忽略（moduleCondition 无 labelGroup 分支→toSql 返回 empty→appendFilterGroup 跳过），与引擎侧正常集合求值不同，属记录页与看板轨潜在口径差异，待独立工单评估；(b) year/month 操作符裸值格式两轨解析不同（引擎 parseDateTimeBoundary 收完整时间戳 vs SQL Integer.parseInt/YearMonth.parse 收裸值），看板过滤器实际发送完整时间戳故为休眠分歧。
- 收尾状态（2026-08-21）：**四项收尾任务完成三项**——①MirrorTable 豁免决策 ✅（见上）；②基类实时状态三兄弟模板化：评估后**决策跳过**（接口 default 化需各板暴露 runtimeSupport 访问器，收益每板约15行低于 Map 重载语义差异风险，签名级重构无行为收益）；③清理验证 ✅（全库 grep 零残留，5 板全走 StatisticFilterEngine.compile）+ architecture.md 引擎契约 ✅ + decisions.md D-07 ✅ + 全量回归 47 绿 ✅ 全部推送。**唯一剩余：SQL↔Engine 平价契约测试**——设计已定：复用 `SqlPushdownRealChainTest` 的真实 PG 夹具基础设施，对同一批 issue_fact 行分别执行 IssueFactRecordRepository SQL 路径与 StatisticFilterEngine 内存路径，断言同结果集；覆盖操作符矩阵与金标同规格。预计半天，需独立工作单元执行。
- 当前进度（2026-08-21 决策记录）：**MirrorTableOverviewBoardService 正式豁免迁移**——其筛选为展示层汇总行方言：大小写敏感 eq/contains、含 notContains 与数值操作符（matchesNumber）、独立日期解析（YearMonth/isEqual 含边界），与事实看板引擎语义本质不同。强行统一需引擎支持大小写开关+NUMBER+双日期解析，会稀释'单一语义'目标或改变用户可见行为，违背零行为变化约束。该板保留自有 matches* 家族（约100行，属领域逻辑而非复制粘贴问题域）。
- 历史进度（2026-08-21 三次更新）：**通用筛选族迁移已全部完成 5/5**——含最大板 SystemTestDefectSummary ✅7e0c3565（14 方法家族删除，testingPhase 经 override+phaseValueCache 闭包保留，EffectiveFilterGroup 双段分别编译 and 组合，matchesModuleDirectoryFilters 同步转引擎子组）。此前：DefectSummary ✅35fae152、DefectCause ✅308fd5e2、ResponseEfficiency ✅db4b1f46、DelayIssues ✅63f46044。ByFunction 无筛选族跳过。——DefectSummary ✅35fae152、DefectCause ✅308fd5e2、ResponseEfficiency ✅db4b1f46、DelayIssues ✅63f46044（引入 multiValueWithCondition 条件感知取值器）。范围再修正：CustomerIssueByFunctionBoardService **无通用筛选族**（仅 matchesTestingPhase 领域过滤），无需迁移；MirrorTableOverviewBoardService 有家族但行类型为 Map<String,Object> 且含数值操作符（matchesNumber eq/gt/lt 等），**迁移前置条件为引擎新增 NUMBER 字段类型**，独立小任务延后；SystemTestDefectSummaryBoardService（14 方法，最大板）**待迁移**，无完整既有单测需从零建金标夹具，严格金标先行。金标基类 AbstractStatisticBoardGoldenMasterTest 已抽取。每板闭环：金标两连跑→迁移删家族→statistics 全量回归→独立提交推送。已知坑：操作符白名单抛 IllegalArgumentException；部分板无 exportIssueRecordsWorkbook；迁移后补 java.util.Objects import；校验层丢弃空值条件并拒绝未知字段/操作符。待办：SystemTestDefectSummary 迁移、MirrorTable（前置：引擎 NUMBER 类型）、SQL↔Engine 平价契约测试、基类实时状态三兄弟模板化、清理验证、architecture.md/decisions.md 同步。
