...
质量看板 / 其他看板对齐老平台 `PageNinePersonalQuality/NinePersonalQuality.vue`。顶部指标来自老平台 `/quality/getAllQualityData` 语义，包含需求评审缺陷密度、设计评审缺陷密度、CC 代码走查缺陷密度、DGM 代码走查缺陷密度、集成测试通过率、发布缺陷遗留率、开发缺陷遗留率和新发缺陷修复率；下方图表对齐老平台按走查人统计代码走查缺陷密度、按被走查人统计代码走查缺陷密度、按修复人统计缺陷数、代码提交频次、指派人剩余缺陷数量。代码走查图表在兼容模式读取老平台 MySQL 兼容快照，非兼容模式读取 `merge_request_fact`；系统测试/议题类图表读取 `issue_fact`。
...
5. 系统测试缺陷汇总、议题阶段统计、申请延期缺陷分析等统计聚合页，必须先按父级阶段查表展开启用的子级测试阶段，再按老平台 `SpiderIssueDataQueryBuilder.setTestingPhases` 语义用 `testing_phase LIKE %具体测试阶段%` 匹配事实层 `testing_phase`。缺陷原因分析是页面专用例外，老平台入口走 `SpiderIssueDataDAO.findAllByCauseExist`，CrownCAD 项目按展开后的具体测试阶段集合精确匹配 `testing_phase`。系统测试议题查询、系统测试非法数据等记录/非法类页面，必须按老平台记录入口使用展开后的具体测试阶段集合匹配 `testing_phase`，不得再用 `system_test_label` 或 `label_names` 兜底扩大范围。其他只切换里程碑的页面，默认只展示启用父级阶段，不直接把子级轮次混入父级下拉。
