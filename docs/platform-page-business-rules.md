...
18. 统计类页面下钻明细中的议题编号、合并请求编号等 GitLab 跳转必须按事实记录的 `sourceInstance + projectId + iid` 生成，并使用对应数据源配置中的 GitLab Web 访问地址。不得只依赖平台当前 host、全局默认 `localhost` 或不带来源实例的项目路径猜测；缺少 GitLab Web 地址或项目路径时应降级为普通文本，不能生成错误链接。
...
1. 代码走查非法数据默认对齐老平台 `codeThroughDataTable`：进入页面默认数据源为 `CC`，项目名称筛选默认 `CrownCAD`，查询合并状态为 `MERGED` 且合并时间晚于 `2024-04-01 00:00:00` 的合并请求，并排除模块名为 `无需标注` 的记录。该页不是议题记录页，不能套用 `projectId=325`。
2. 目标分支筛选按老平台 `getDevBranch(name, targetBranch)` 行为处理：选择 `source/name = CrownCAD` 或 `DGM` 且目标分支为空时，默认补 `dev`；其他数据源非法数据查询不得隐式限制到 `dev`。
3. 非法类型包括未标注项目名、未标注模块名、无代码走查、未进行代码扫描、静态扫描问题未关闭、代码注释量未达标、静态扫描失败、注释率分析工具 Clang 分析错误和 GitLab 接口报错。筛选项可显示老平台文案，但后端必须映射到事实字段统一判断；“未进行代码扫描”只匹配老平台 `sonar_qube_result = 未进行代码扫描` 的字面值，不得把 `NOT_SCANNED`、`UNSCANNED`、`未扫描` 等新平台技术状态码扩展进默认非法口径。
...
6. `静态扫描问题未关闭` 对齐老平台 `bug_count_result = 静态扫描问题未关闭`，不能仅因扫描问题数大于 0 就判定为非法。
7. `GitLab 接口报错` 对齐老平台 `sonar_qube_result / target_branch / assignee` 三处来源。新平台事实层映射后应同时检查扫描状态、目标分支、走查人/负责人展示字段，避免字段拆分后漏判。
8. 页面顶部“项目”切换必须对齐老平台 `projectName` 字段，用于筛选代码走查所属项目或版本；`source/name` 只用于 CrownCAD、DGM 数据源切换，不能用 GitLab `projectId` 替代 `projectName` 版本筛选。
9. 页面条件中的“被走查人”按老平台 `author` 字段筛选；不能因为新平台内部参数名为 `owner` 就改查 `owner_name`。责任人维度属于代码走查多元看板，不替代非法数据页的被走查人筛选。
10. 代码走查非法数据的详情展示必须至少覆盖老平台展开行字段：走查编号、所属项目或数据源、走查时间、模块名、被走查人、走查人、被指派人、合并时间、合并人、合并目标分支、走查工作量、新增走查代码行数、缺陷数、代码走查速率、代码走查缺陷密度和代码走查效率。新平台可以追加项目名称、代码库、非法判定、扫描状态和分类缺陷数等增强字段，但不能删减老平台已有字段。
...
8. 评审数据管理页“新增评审”和“新增评审问题”中的候选值必须优先来自 GitLab 镜像库全量数据。新增/编辑评审里的项目名称来自 `ods_gitlab_labels` 中的 `项目：xxx` / `项目:xxx` 标签，模块来自 `ods_gitlab_labels` 中的 `模块：xxx` / `模块:xxx` 标签；下拉展示和保存值都只保留冒号后的 `xxx`。人员来自 `ods_gitlab_users`。新增/编辑评审里的评审版本下拉固定提供 `V0.1`、`V0.2`、`V0.3`、`V0.4`、`V0.5`、`V0.6`、`V1`、`V2`、`V3`，并允许用户手动输入其他版本号，例如 `V10.1`。历史评审导入数据只作为列表筛选和非标签来源字段的补充兜底，不能把新增评审可选范围限制为已经导入过的评审记录。
9. 编辑评审记录时，评审专家表和清单中的系统占位问题项必须同步：新增专家可自动补充“未评审/独立评审/无问题/待评审”的空占位项；删除专家时只允许移除该专家尚未填写过的系统占位项，已经录入具体位置、问题、建议、责任人、不采纳原因、状态或非 0 工作量的真实清单项必须保留。
