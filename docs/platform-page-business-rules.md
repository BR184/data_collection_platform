...
5. 年月日分隔符允许使用：`.`、`,`、`，`、`、`、`/`、`·`、`` ` ``、`年`、`月`、`日`。
6. 客户问题非法模板校验只接受一个或多个 `CCyyyyRn` 版本标识；多个版本以 `&` 分隔，例如 `CC2026R4 & CC2026R5`。任一成员不符合格式时判定调研模板非法，但不能据此清空事实字段。`planned_merge_version_branch` 只折叠最新模板字段的空白并保留其余来源文本；页面主表和详情按 `&` 渲染标签，Excel 使用同一事实文本。
