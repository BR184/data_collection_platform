...
7. 客户问题记录类页面（`CC_PRODUCT议题`、客户问题延期记录、客户问题缺陷非法数据等）必须按老平台 `IssueStaticDataController.filter` 的 CC_Product 分支对齐展示数据筛选规则：默认 `projectId=325`，默认 `created_at/submission_date >= 2026-01-01`，顶部范围使用 `milestone_title` 而不是系统测试 `testing_phase`，并复用客户问题公共排除规则。导出、筛选候选、分页总数和页面列表必须复用同一套规则。
