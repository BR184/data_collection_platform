...
标签组是将多个同维度标签聚合为一个可复用筛选单元的能力，用于提升条件配置效率和统一分析口径。详细方案见 `docs/plans/2026-06-10-platform-label-group-filter-design.md`；实现前必须先补齐 `docs/plans/2026-06-10-label-group-dimension-matrix.yml`，明确标签维度、候选值来源和页面兼容矩阵。Markdown 矩阵只作为人工说明，YAML 是实现权威源。

1. 标签组只能属于一个标签维度，例如模块、项目、评审负责人、评审专家、议题处理人、客户问题处理人、目标分支、里程碑、轮次、测试阶段、严重程度、紧急程度、缺陷原因或延期原因。人员类维度必须按页面事实语义拆分，不允许使用泛化的 `owner` 跨页面复用。
2. 标签组成员只能来自该维度下已归一化的事实字段值或老平台规则明确识别的派生值，不能由用户手动输入任意字符串。
3. 标签组不能跨维度组合条件；“评审负责人=张三 AND 项目=CrownCAD AND 模块=草图”属于页面筛选条件组合，不属于一个标签组。
4. 页面应用标签组时，语义固定为“字段 IN 标签组成员”。例如“模块 = 核心业务系统（标签组）”展开为“模块 = CRM OR 模块 = OA OR 模块 = ERP”。
5. 页面能否应用某个标签组，由页面是否支持该标签组所属维度自动决定；不让用户手动填写适用页面、字段范围、适用对象或 Hash。
6. 业务页面只应用已保存标签组；标签组创建、编辑、删除和成员维护放在系统设置中。
7. 指标字段、统计表头、占比、合计、缺陷密度、工作量、页面说明和规则说明不得作为标签组成员候选。
8. 静态标签组由用户从系统候选值中手动维护成员；动态标签组由系统按已确认规则在同一维度内自动计算成员，动态规则不得输出跨维度条件。
9. 应用标签组后的查询、导出、下钻、排序和二次筛选必须复用页面原有事实层查询口径。
10. 不得复用已废弃的对象分群、语义标签组、跨字段筛选方案或相关旧运行时命名。
11. 标签组成员候选必须过滤空值、`未设定...`、`未标注...`、`GitLab接口报错` 等占位值；包含 ` & ` 的组合值按现有筛选规则拆分后去重。
12. 标签组第一版为全局共享筛选项，不绑定个人、页面或源实例；个人收藏、最近使用、角色默认视图、跨字段筛选方案和业务场景模板独立设计，不能复用标签组表。
13. 标签组成员变化后，历史数据重新查询按当前成员展开；如果业务方要求历史口径冻结，必须等待业务事实历史快照方案落地后再扩展。
14. 单个标签组成员数量上限为 200，超过时提示拆分，避免过长查询拖慢页面。
15. 同字段普通值和标签组成员折叠为同一个 `IN` 条件时，去重后的最终值数量上限同样为 200；超过时返回业务错误，不能静默截断。
16. 客户问题闭环状态中 `需求如此` 和 `设计如此` 为等价值。标签组成员保存规范值 `需求如此`，候选值只展示 `需求如此`，查询展开时同时匹配 `需求如此` 和 `设计如此`。
17. 导出文件需要展示标签组名和导出时展开的成员快照，例如“模块 = 核心业务系统（标签组：CRM、OA、ERP）”，该快照只写入导出文件，不写入数据库。
18. 多值字段应用标签组时，匹配语义为“字段值集合与标签组成员集合相交不为空”。例如评审专家为“李四、王五”的记录，应用成员为“张三、李四”的评审专家标签组时应命中。
19. 标签候选值和标签组展开接口必须显式支持 `pageKey` 和可选 `sourceInstanceId`；不传 `pageKey` 时按全部兼容页面和全部源聚合，传入 `pageKey` 时按该页面数据范围过滤，不能从 session 隐式读取页面或源实例状态。
...
11. 如果页面支持标签组筛选，是否只展示当前字段维度可用的同维度标签组。
