...
5.1 兼容模式-MatchMode 只读取老平台 Mongo 的原始评审记录、问题清单和评审规模明细；列表、导出、质量看板和系统测试横向对比中的 `reviewDefectDensity`、`reviewEfficiency`、`reviewRate` 与正式模式共用同一套有效问题重算口径。不得直接展示 Mongo `reviewReport` 中可能陈旧的派生密度、效率、速率字段；只有缺少问题明细时，才允许用老平台 report 中的基础计数字段兜底后重新计算。
