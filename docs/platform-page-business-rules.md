...
| 评审数据管理问题列表 | 导出问题列表 | 导出问题列表 | `评审问题详情.xls` |
...
15.0.1 对齐老平台的议题、客户问题和代码走查页面，“最近同步”和任务执行时间优先来自老平台 Mongo `ScheduledTimeRecord`，查询规则与老平台 `/scheduledTimeRecord/findByParam` 一致：按 `typeName`、`typeValue`、可选 `projectId` 查询并按 `startTime` 倒序取最新一条。当前映射为：代码走查非法数据 CC 使用 `mergeRequest / CrownCAD / 9`，DGM 使用 `mergeRequest / DGM / 79`；系统测试统计和非法数据按当前测试阶段使用 `issue / {testingPhase} / 9`；客户问题统计、客户问题非法数据、CC_PRODUCT 议题和延期问题使用 `issue / CCProduct / 325`。若没有匹配的老平台任务记录，才允许回退到 GitLab 镜像最近同步时间。
...
9. 导出文件对齐老平台 `ExportModuleAndCauseTableRow` 两级表头，不增加新平台中间分组层；字段顺序以该 DTO 的 `@ExcelProperty` 声明顺序为准。
...
6. 老平台该页面没有导出按钮，新平台也不得展示导出按钮；不能用平台通用统计导出替代老平台不存在的功能。
...
1. 代码走查非法数据默认对齐老平台 `codeThroughDataTable`：进入页面默认数据源为 `CC`，老平台 `name/nameSearch`（新平台兼容表字段 `repositoryName/repository_name`，页面展示为“所属项目”）默认 `CrownCAD`，`projectName/project_name` 是独立的“项目名称”筛选，不能等同于 `name`。所有非法类型判定前必须先限定合并请求状态为 `MERGED`，且合并时间晚于 `2024-04-01 00:00:00`。基础范围只包含已合并 MR；`opened`、开放中、草稿开放中、`closed` 和已关闭 MR 一律排除，不能被项目、模块、关键字、非法类型、快速筛选或高级筛选重新带回。基础范围必须排除携带 `无需走查扫描` 标签的记录，并排除模块名或项目名为 `无需标注` 的记录。该页不是议题记录页，不能套用 `projectId=325`。
