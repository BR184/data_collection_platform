...
4. “议题测试阶段定义”必须支持父级阶段和子级测试阶段的新增、编辑、删除、启停和自由排序。维护页默认展示全部父级阶段，停用只是开关状态，不能导致记录从维护页消失；子级测试阶段名称默认隐藏，只有选择某个父级阶段后才显示。父级阶段列表只展示启用项时，其排序就是系统测试和客户问题相关页面中“测试阶段/切换里程碑”下拉的展示顺序；子级测试阶段排序就是选择某个父级后展开到具体轮次时的匹配顺序。
