...
5.1 兼容模式 match mode 下直接展示老平台 Mongo `reviewReport` 中已经保存的 `reviewDefectDensity`、`reviewEfficiency`、`reviewRate` 数值；老平台页面对这些字段使用 `toFixed(2)` 展示。若历史记录缺失对应字段，才按老平台展示口径用四舍五入两位小数兜底，不能套用普通新平台记录的向下取整规则。
