...
12. 评审前临时兼容模式 `MatchMode` 默认开启，用于外部注释率工具和静态扫描工具正式接入新平台前的过渡。兼容模式的开关、老平台 MySQL 连接配置和导入表白名单只维护在系统设置 / 数据库设置对应的独立表 `code_review_match_mode_db_settings`，不得继续混入 GitLab 数据镜像设置。系统设置页必须从当前 MySQL 数据库动态读取全部基础表供用户选择，用户选择的任意表导入到通用原始快照表；其中代码走查兼容表默认 `spider_crowncad_data`，被选中时额外刷新独立兼容表 `code_review_match_mode_records` 供代码走查非法数据页读取。注释率、SonarQube 等评论区汇总数据不再从 MongoDB 读取，后续由老平台 MySQL 或单独维护的外部汇总表提供。兼容导入不写入或改动新平台事实表、GitLab ODS 镜像表或评审数据表；关闭后恢复读取新平台 `merge_request_fact`。兼容模式开启时页面不提供“刷新最新数据”按钮。所有兼容实现必须标注 `//兼容模式-MatchMode`，评审后按临时代码整体删除。
