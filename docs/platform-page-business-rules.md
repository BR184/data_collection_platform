...
10.1 表格排序的空值规则（与占比显示同口径）：单元格没有可比较数据时（比率分母为 `0` 显示 `/`、外部平台未提供计数、时间或文本为空），该值不参与升序/降序的大小比较，恒排末尾；真实 `0` 是有效值，必须与“无数据”分开。统计看板与 BI 看板共用同一置底比较器实现，不得在局部比较器用 `?? 0`、`?? -1`、`?? 101` 等哨兵把无数据坍缩成数字（那会导致无数据与真实 0 交错或随方向翻到顶部）。
...
4. 客户问题默认排除：
   ...
   - 按领导 2026-09-08 确认的新口径，客户问题延期问题与缺陷汇总看板在统计层排除建议类问题：严重程度归一为建议类或类别包含“建议”的议题，不计入响应延期、解决延期、模块总缺陷数、修复率、关闭率、占比等常规指标（详见 5.2、5.3）。判定口径与系统测试侧 `SuggestionMetricSupport` 三口径一致。此前“客户问题统计不排除建议类”是与老平台对齐的旧口径，已被本条替代；事实层 `is_excluded` 仍只排除关闭后的申请否决/需求如此/设计如此，`CC_PRODUCT议题` 等记录页不受本条影响。
...
5. P1、P2、P3 缺陷关闭率统一按“当前优先级已关闭缺陷数 / 当前优先级总数”计算；客户问题缺陷汇总统计及其导出必须在各优先级修复率后输出对应关闭率，分母为 0 时显示 `/`。
6. 建议类缺陷按领导 2026-09-08 确认的口径统计（对齐 4.3 第 5 条系统测试机制）：严重程度归一为建议类或类别包含“建议”的议题只进入“建议类缺陷(个)”列，不计入一级、二级、三级、P1/P2/P3、模块总缺陷数、缺陷占比、延期占比、修复率、关闭率、未关闭数量、申请延期、复测未通过、新发议题、遗留率等常规指标；“模块总缺陷数(个)”下钻明细与单元格数字使用同一口径，不含建议类。“全量议题数据”导出属于可见层数据，仍包含建议类议题。
7. 客户问题“全量议题数据”以老平台议题字段顺序为基础，在“修改该缺陷可能影响的功能”后依次输出“已知的受影响功能”“新识别的受影响功能”，值遵循系统测试缺陷汇总第 6 条的“是/否/--”语义。测试阶段直接使用事实字段；事实为空时统一展示“未设定测试阶段”，不得从项目名、里程碑或其他标签猜测阶段。
...
14. 平台可按延期事实字段与 GitLab 上的 `响应已延期` / `解决已延期` 标签做差异写回，但写回必须同时满足两个开关：后端全局开关 `CUSTOMER_ISSUE_DELAY_LABEL_WRITEBACK_API_ENABLED=true`，以及对应数据源 `delay_label_writeback_enabled=true` 且配置了 `web_base_url` 和 `api_token`。写回关闭时仍刷新并监控延期事实，不调用 GitLab 写接口。写回开启时必须先增量刷新 `issues`、`notes`、`label_links`、`labels`，并等待该来源实例的 ISSUE 事实发布收敛（已无未发布目标），再把差异写入 PostgreSQL 持久化队列，由后台 worker 调用 GitLab API。该等待由运行终态事件推进，不设固定超时：镜像耗时再长也只推迟本轮写回入队，不会因超过某个阈值而整轮跳过，也不会占用调度线程。队列任务只允许通过 `add_labels` / `remove_labels` 增删 `响应已延期`、`解决已延期` 两个标签，不得覆盖、删除或重排其他标签；同一议题只保留一条未完成写回任务。GitLab 5xx、429 和网络错误可按退避策略重试，401/403/404/400 直接标记为不可自动重试。内网开发测试包默认关闭全局开关，即使页面误开数据源写回或误填 token，也不得调用 GitLab 写接口。未显式批准真实写回前，只验证事实字段、统计结果、去重差异和日志，不触碰 GitLab 真实标签。
15. 延期问题统计排除建议类问题（领导 2026-09-08 确认）：严重程度归一为建议类或类别包含“建议”的议题不计入响应延期和解决延期的 `P1`、`P2`、`P3`、总计计数及其下钻明细。判定口径与系统测试侧 `SuggestionMetricSupport` 三口径一致。
...
13. 清单里新增或编辑评审问题时，问题状态均必选且候选项不含"未评审"；"未评审"仅由系统在按评审专家自动创建占位项时使用，不得手动选择。新增问题时若该专家存在未填写的系统占位项，视为对该占位项的首次填写（复用其条目并写入所选状态）；此后该专家的后续问题一律新增独立条目，同一评审专家可有多条评审问题。含真实内容但停留在"未评审"状态的存量条目不会被后续新增覆盖，需人工编辑补选明确状态。
14. 评审数据管理页必须复用系统测试议题查询的展开快速筛选模式，并与“筛选条件”高级条件筛选同时生效；快速筛选、条件筛选、导出和分页排序必须复用同一套记录查询参数。
15. 评审数据管理页“是否达标”判断：设计说明书评审按评审缺陷密度介于[0.3~0.8]判定，其余评审类型（含需求说明书评审）按[0.2~0.6]判定。原“统一[0.2~0.6]对齐老平台”口径自 2026-09-09 起被设计评审质量目标更新替代；实现统一收敛于后端 `ReviewDataReachStandardRule`，兼容态与正式态、列表读取与排序 SQL 同口径。
