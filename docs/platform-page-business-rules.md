...
12. 评审前临时兼容模式 `MatchMode` 默认开启，用于外部注释率工具和静态扫描工具正式接入新平台前的过渡。兼容模式的开关、老平台 MySQL 连接配置、老平台 MongoDB 连接配置、MySQL 导入表白名单和 MongoDB 导入集合白名单只维护在系统设置 / 数据库设置对应的独立表 `code_review_match_mode_db_settings`，不得继续混入 GitLab 数据镜像设置。老平台代码走查数据中 CC 默认来自 MySQL `gitlab_spider` 库，DGM 默认来自同主机同账号的 `gitlab_spider_dgm` 库，二者同样读取 `spider_crowncad_data` 并写入新平台独立兼容表 `code_review_match_mode_records`，通过 `source_instance=cc/dgm` 切换展示。系统设置页必须从当前 MySQL 数据库动态读取全部基础表供用户选择，用户选择的任意表导入到通用原始快照表；其中代码走查兼容表默认 `spider_crowncad_data`，被选中时额外刷新独立兼容表 `code_review_match_mode_records` 供代码走查非法数据页读取。评审数据来自老平台 MongoDB `spider` 库，默认集合为 `reviewReport` 和 `problemDetail`，必须通过 MongoDB 集合白名单选择并写入独立兼容表，不写入正式评审数据表。兼容导入不写入或改动新平台事实表、GitLab ODS 镜像表或正式评审数据表；页面是否读取兼容表由 `review_data_read_mode` / `code_review_read_mode` 独立控制，不能仅凭全局兼容模式开关判断。兼容模式自动同步按“上次同步任务结束后再等待 10 分钟”触发，不按现实时间整点/整 10 分钟触发；同步过程中页面继续读取上一次已完成同步的稳定兼容表数据。系统设置页需要提供“老平台数据转正式数据”入口，用户输入确认文本后，可将老平台评审数据写入正式评审表、将老平台代码走查数据写入正式 `merge_request_fact`；重复导入按稳定来源键更新，不追加重复数据。单页刷新按钮必须跟随当前页面读源：读取兼容表时刷新老平台源数据并同步兼容表；读取正式表时刷新正式事实层，若当前正式事实来自老平台转正式数据，则先刷新老平台对应 MR 再重新提升到正式事实。所有兼容实现必须标注 `//兼容模式-MatchMode`，评审后按临时代码整体删除。
...
5.1 兼容模式 match mode 下直接展示老平台 Mongo `reviewReport` 中已经保存的 `reviewDefectDensity`、`reviewEfficiency`、`reviewRate` 数值；若历史记录缺失对应字段，按同一套四舍五入保留两位小数口径兜底。
