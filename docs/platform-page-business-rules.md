...
1. 代码走查非法数据默认查询合并状态为 `MERGED` 且合并时间晚于 `2024-04-01 00:00:00` 的合并请求，并排除模块名为 `无需标注` 的记录。页面默认数据源为“全部数据源”，不能在未选择数据源时自动落到 CrownCAD。
...
1. 客户问题默认统计 `CC_Product` 项目 `325` 自 2026-01-01 之后创建的议题；项目范围只能来自 `project_id=325` 或归一化后的 `project_name=CC_Product/CC_PRODUCT`，不能因为里程碑、测试阶段或标签文本包含 `cc_product` 就反推出客户问题范围。
