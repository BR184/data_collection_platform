...
- [下一步] 将自动事实刷新身份修复与权威关系同步修复部署到内网；部署后执行一次成功 `FULL_COMPENSATION_SCAN` 及其全量事实发布清除历史 ODS/事实漂移，再用隔离 Issue 标签从“未修复”变为“已修复/完成”，核对父子运行、定向事实、快照和 CC2026R4 缺陷汇总闭环。后续正常自动更新不得绑定全量补偿。
...
- [下一步] 按 [GitLab 物理删除同步覆盖审计与修复交接](plans/audit-gitlab-physical-delete-reconciliation.md) 完成内网发布验收：部署后执行一次成功 `FULL_COMPENSATION_SCAN`，等待 ISSUE/MERGE_REQUEST/INTEGRATION_TEST 事实与快照刷新成功，再以隔离测试对象核对标签、指派、评论、MR 指标和根实体删除在 ODS tombstone、事实及页面/导出三层均无残留。
...
- [下一步] 将同步运行时部署到具有约 270 万行真实 GitLab 数据的内网后，固定相同数据范围分别测量 2/4/6 worker 的分页吞吐、DIRECT 池等待数、GitLab 负载和失败率；同时在后台全量补偿运行中提交增量与单表刷新，验证其在分页边界让行、低延迟完成且全量最终从 cursor 恢复。失败任务须能用 `runId/runDbId/taskId` 串联日志。
...
- [风险] 本地样本不能替代约 270 万行内网源库的容量基准；2/4/6 worker 的推荐值和全量补偿绝对耗时仍须以内网重复测量为准。本轮不改变 ODS、事实或统计业务字段，部署后不要求为该运行时重构执行 GitLab 全量补偿或事实重建。
- [风险] 代码已覆盖 `notes`、Issue/MR metrics、根实体精确删除和 GitLab 16.11 `resource_label_events` 映射，但历史 ODS/事实漂移不会由部署本身自动消失；首次部署仍须完成一次 `FULL_COMPENSATION_SCAN` 及其全量事实刷新。未收到父资源增量或 System Hook 的任意物理删除继续由该周期性全量对账兜底，不能用单独事实重建或页面过滤替代。
...
- [风险] `check_schema_flyway_drift.py` 当前仍报告 13 张表和 34 个索引仅存在于既有 Flyway 迁移序列；本轮新增代码评审指标结构已与 `schema.sql` 对齐，其余全局 schema 收敛应作为独立工作单元处理。
