...
- [目标] 完成 GitLab 日常增量与手动刷新中的物理删除探测和精准派生发布收口：来源/事实/工作区目录、批量权威范围、统一 `SCAN -> RECONCILE` 屏障、事务内版本化 outbox、根版本 fencing、有界事实批次、稳定范围 generation、publication fence 和自动增量删除页让行均已落地；当前按 `docs/plans/verify-package-deploy-latest-to-20001.md` 执行最终门禁、721 保数据升级演练和内网真实删除验收。正常链路不得依赖全量补偿或全量事实刷新。
