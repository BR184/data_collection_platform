...
## 2026-08-20 本地工作树与远端基线清理

- [完成] 以 `D:/projects/datacollectionplatform-main` 为远端基线同步 29 个公共/新增文件，删除本地远端不存在的 17749 个过期源码、计划、验证产物、日志、构建产物和数据库导出；`.git`、本地环境、依赖工具链和活动 PostgreSQL 数据目录按运行例外保留。
- [验证] 远端 1635 个源码文件全部存在且 SHA-256 一致；旧 `BiCodeReviewCompatibility*` 生产/测试类已删除，活动代码不再引用旧同步表，后端 1157 项测试零失败、零错误、1 项条件跳过，前端 115 个测试文件/422 项零失败，ESLint、TypeScript 和生产构建通过。
- [验证] 新代码已在 `18080` 启动并返回健康状态，`18181` 返回数据采集平台首屏，前端 `/api/auth/current` 代理返回游客态，浏览器刷新后的控制台错误/警告为空。
- [风险] 本地 `15432/qaflex` 的旧 BI 兼容表仍有 30508 行、同步状态表有 1 行，与远端 `V20260813_01` 的“当前均为空”前提不一致；本轮未执行该破坏性迁移，验证后端以 `SPRING_FLYWAY_ENABLED=false` 启动。正式启动新迁移前必须先确认旧数据迁移/留存方案。

## 2026-08-21 客户问题 325 里程碑目录为空修复

- [完成] 根因已确认：`V20260727_02` 迁移执行时 `issue_fact` 为空，只能建立空目录；运行期目录协调器在无组时直接返回；事实 SQL 还曾在里程碑源结构缺失时回退为空字符串，掩盖源数据契约错误。
- [完成] 已实现运行期首次非空事实初始化、已有启用同键组的缺失成员补齐、标准 CC 版本空格/大小写归一化；保留人工组的显示名、顺序、启用状态和成员边界，不创建额外组或启用停用组。
- [完成] 新增 `V20260821_01__bootstrap_customer_milestone_scope_catalog.sql`，仅在 325 `MILESTONE` 目录无组且已有事实时回填；源结构守卫现在强制检查 `ods_gitlab_milestones` 与 `ods_gitlab_issues.milestone_id`，移除空里程碑事实回退。
- [验证] 协调器 3 项、迁移升级 1 项、目录/业务键 6 项、源结构与事实 SQL 10 项、Flyway 20 项、事实构建 3 项通过；Java 21 编译、Checkstyle、SpotBugs、后端全量 1164 项（失败 0、错误 0、跳过 1）通过，文本空白、运行产物位置、迁移不可变性、破坏性迁移和 `git diff --check` 通过。
- [完成] 已生成并校验更新包 `qaflex-update-20260821T064536Z-3223b8388269.tar.gz`（SHA-256：`dafa29bcfe0647d9ac7ee262c460c182f33748178f6e57878cb12fc5f7373664`），30001 仅更新 backend/frontend 镜像；PostgreSQL 容器 ID、卷和保护表行数保持不变，Flyway 到 `20260821.01`，两项应用均 healthy。
- [验证] 30001 HTTP/浏览器未登录态正常，实例专属 CSRF Cookie 生效，页面控制台无错误/警告；本地 18080 已以 `SPRING_FLYWAY_ENABLED=false` 重启到最新代码，18181 页面和控制台检查通过，本地事实及 325 目录行数未变。
- [阻塞] 30001 当前库没有 `gitlab_sync_configs`、`sys_table_registry` 或任何 ODS 表，`issue_fact=0`、325 里程碑目录为 0 组/0 成员，因此本轮不能执行真实 `issue` 事实重建或宣称客户问题候选已通过；需先在该实例恢复源配置并同步 ODS。
- [风险] API 漂移检查剩余 6 个已删除 BI 路由映射缺项属于工作树既有问题，本工作单元不处理；后端日志中的外部 MySQL/Mongo/CAT 连接失败属于当前依赖未启动/不可达，本地重启后既有 GitLab 配置的自动同步也因源库账号认证失败而失败，但未改变事实行数，不影响本次应用健康和页面未登录态。
...
- [历史/仍有效] 评审普通类型、代码走查边界和产品版本归一化以 `docs/decisions/ADR-006-product-version-normalization-and-review-domain-boundary.md` 为决策来源。
