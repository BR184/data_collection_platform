...
- [阻塞] `172.22.10.115` 的 22/20001/20002 TCP 可达，但 Windows OpenSSH、Git OpenSSH 和 `ssh-keyscan` 均在主机密钥交换前被远端关闭；现场 SSH 服务或访问策略恢复前，无法读取唯一 Compose、生成预部署备份和执行 20001/20002 保数据升级。该失败发生在用户/密钥认证前，不通过猜测账号或绕过脚本处理。
