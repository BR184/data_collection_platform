...
- [限制] 2026-08-04：首次全量后的自动 `FACT_REFRESH` 是正常且必要的事实/投影发布；自动同步只依据 `last_incremental_sync_at`，而全量只推进 `last_full_sync_at`，因此自动同步开启且增量水位为空时可能额外排入“刷新最新数据”。现场规避为首次全量及事实刷新成功前关闭自动同步，完成后再开启；后续统一调度水位并避让活动全量。
- [限制] 2026-08-04：取消采用协作式机制，不主动中断远程 JDBC 查询；默认外部查询超时为 120 秒，权威范围和事实处理按批次收尾，人工中止出现两三分钟延迟属于当前边界。后续需传播取消信号并在可行时主动取消远程 `Statement`，同时避免未初始化增量水位导致取消后立即重提。
