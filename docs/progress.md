...
- [下一步] 用户使用 release `20260803T082443Z-2ab4bea80a7b` 与同 ID 的 20001 包外配置执行不带 `-v` 的 `docker compose down`、加载新业务镜像并 `up -d`；随后核对目标镜像、Flyway `20260803.01`、后端 UP、前端 200 和原 PostgreSQL volume，再按发布清单完成 `all` 事实重建、快照预热及真实业务页面验收。
