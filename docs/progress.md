...
- [目标] 旧发布 `20260803T054734Z-066761e14130` 已被后续 `CC_PRODUCT` 与严格缺陷原因修复替代；当前按 `docs/plans/verify-package-deploy-latest-to-20001.md` 将已验证候选形成干净提交，并以 721 直接基线重新生成更新包及引用新 release ID 的 20001 完整 Compose/`.env`。
- [目标] 完成 GitLab 日常增量与手动刷新中的物理删除探测和精准派生发布收口：来源/事实/工作区目录、批量权威范围、统一 `SCAN -> RECONCILE` 屏障、事务内版本化 outbox、根版本 fencing、有界事实批次、稳定范围 generation、publication fence 和自动增量删除页让行均已落地；当前按 `docs/plans/verify-package-deploy-latest-to-20001.md` 重新打包，随后由用户执行 721 保数据升级并完成内网真实删除验收。正常链路不得依赖全量补偿或全量事实刷新。
...
- [下一步] 新 release 与匹配的 20001 包外配置生成并完成审计后，用户执行不带 `-v` 的 `docker compose down`、加载新业务镜像并 `up -d`；随后核对目标镜像、Flyway `20260803.01`、后端 UP、前端 200 和原 PostgreSQL volume，再按发布清单完成 `all` 事实重建、快照预热及真实业务页面验收。
- [下一步] 继续补充关键交互和导出的定向回归测试，避免页面仅在切换路由或手动刷新后才恢复数据。

## 阻塞与风险

- [阻塞] `172.22.10.115` 的 22/20001/20002 TCP 可达，但 Windows OpenSSH、Git OpenSSH 和 `ssh-keyscan` 均在主机密钥交换前被远端关闭；该问题只阻塞 AI 的现场只读核验与自动部署，用户已确认自行使用受审计配置执行保数据升级。失败发生在用户/密钥认证前，不通过猜测账号或绕过脚本处理。
...
- [验证] 2026-08-03：固定提交 `2fd5c34e` 的隔离验证工作树已完成物理删除精准发布真实 PostgreSQL 全链回归。`INCREMENTAL_SYNC`、`TABLE_REFRESH` 两种入口均在同一 `RECONCILE` 任务行续页后把已删除一级缺陷标签写为 ODS tombstone，真实事实替换将 `issue_fact.severity_level` 清空并使一级缺陷计数从 1 归零；版本化 outbox、相关投影 generation 与任务全部发布成功，无关项目 generation 不变，未产生 `FULL_EPOCH`。新同步专项 72 项全部通过；完整后端 948 项零失败、零错误、1 项跳过。
