...
- [目标] 对 GitHub `origin/main` 最新提交 `2fd5c34e` 执行冻结代码的全仓测试并统一登记问题，集中修复后完整复验；全绿后按 `deploy/intranet-offline-packaging-standard.md` 以 20001 实例当前 721 成功更新包为直接基线生成保数据更新包，完成隔离升级/回滚/再升级后部署到内网 20001，PostgreSQL 容器与数据卷身份必须保持不变。活动计划见 `docs/plans/verify-package-deploy-latest-to-20001.md`。
- [目标] 完成 GitLab 日常增量与手动刷新中的物理删除探测和精准派生发布收口：来源/事实/工作区目录、批量权威范围、统一 `SCAN -> RECONCILE` 屏障、事务内版本化 outbox、根版本 fencing、有界事实批次、稳定范围 generation、publication fence 和自动增量删除页让行均已落地；当前按 `docs/plans/implement-incremental-delete-detection-targeted-refresh.md` 执行最终门禁和内网 270 万行容量/真实删除验收。正常链路不得依赖全量补偿或全量事实刷新。
...
- [验证] 2026-08-03：系统测试缺陷原因全集污染已按老平台固定模板规则修复：系统测试与客户问题共同使用唯一严格原因归一化入口，缺标题、标题不在首行、粗体改写、缺少原因段、标签和自由文本均不再生成原因，合法模板结果不变。归一化 31 项及事实链、事实构建、客户原因/非法、系统测试多元看板和横向导出 25 项回归通过，Checkstyle 0 违规、SpotBugs 0 问题；本地只读差集确认项目 9 的 73 条非空原因中 67 条合法模板保持、仅 6 条原因全集污染归零，项目 325 的 82 条非空原因全部保持。部署后必须执行 issue 全量事实重建并由新事实来源版本刷新统计快照；未在本地写入重建。此前完整后端套件 943 项中 21 项失败、11 项错误，其中事实链旧契约已修正并通过，其余失败不经过本次归一化入口；`SqlPushdownRealChainTest` 仍在登录初始化阶段失败，`SystemTestIllegalRecordServiceTest` 仍有 4 条既存 SQL/内存分页基线冲突，本任务未修改这些范围外问题。
...
- [验证] 2026-08-03：评审数据页异常根因为 `page_record_snapshots.source_version` 的 `varchar(256)` 限制无法容纳动态评审来源版本；已新增 Flyway `20260803.01` 改为 `text` 并同步初始化 schema。主源码编译、Checkstyle、迁移契约和快照服务定向测试通过，公共库已执行迁移；真实 LDAP Session 页面数据请求待重新建立会话后验收。
- [验证] 2026-08-03：系统设置、兼容模式和权限页面的“页面资源加载失败”已复现为 Vite `504 Outdated Optimize Dep`，与后端评审接口异常无关；`frontend` 开发启动改为 `vite --force` 后，评审数据、数据镜像、数据库兼容模式、权限、标签组和测试阶段六个路由浏览器烟测全部通过。烟测同步补齐当前认证响应和设置接口夹具；未修改业务数据。
...
- [完成] CC_PRODUCT 响应模板计划字段已收敛为分离的事实与校验规则：计划解决时间只保留唯一完整的合法日期；计划合并版本分支只规范化空白并保留最新模板来源文本，客户问题非法模板校验仍只接受以 `&` 分隔的 `CCyyyyRn` 成员，校验失败不清空事实。主表、详情和 Excel 共用该事实字段。
...
- [验证] 2026-07-29：单一 Compose 包通过 25 项打包契约、生成内容真实 `docker compose config`、前端 14 项发布测试/类型检查/生产构建、后端生产包、无缓存镜像、镜像内摘要、Bash 语法、包内/归档校验、后台调度配置测试、Checkstyle 和 SpotBugs。隔离栈从 `100101` 基线依次完成“备份→升级→回滚→第二次备份→第二次升级”；两轮完整/关键表 dump 约 342024/174118 bytes且可由 `pg_restore --list` 读取，`counts.diff` 均为空，Flyway `20260727.04`，后台调度为 true，PostgreSQL ID `e7f1632a...b53707` 与 `.env` SHA-256 `99ff7c...39377` 始终不变，回滚后 Compose SHA-256 精确恢复。运行产物位置门禁仍仅被根目录既有 8 个 `.tmp-*.log` 阻塞，未删除用户文件。

## 下一步

- [下一步] 完成本工作单元的最终差异、静态门禁和定向回归；随后在约 270 万行等价副本或内网维护窗口按活动计划固定数据集测量 2/4/6 worker 与候选 batch，记录删除探测、事实和投影阶段 P50/P95、GitLab/平台负载、连接、堆内存和任务表增长。未达到硬门禁不得上线，也不得通过盲目增加线程掩盖。
- [下一步] 加固周期性全量补偿调度，但不将其绑定到每次自动增量：每日计划改为“已过计划点且当天无成功/活动补偿”的持久幂等判定，只复用活动 `FULL_COMPENSATION_SCAN`；`last_incremental_sync_at` 仅由 `INCREMENTAL_SYNC` 更新。补充普通活动镜像不能吞掉每日补偿、补偿不推迟增量周期和全量事实发布延迟监控回归。
- [下一步] 容量门禁通过后按 ADR-005 部署：停止并清空旧消费者、备份平台库与配置、执行 Flyway，再只运行一次 `FULL_COMPENSATION_SCAN` 及全量事实/投影发布清除部署前历史漂移。随后用隔离 Issue/MR 分别验证自动增量和页面手动刷新能在不运行全量补偿/全量事实的情况下收敛标签、指派、评论、指标和根实体物理删除，并复验无关页面、其他来源实例、兼容模式和外部 API 不变。
- [完成] 已恢复评审“兼容态正式表加未接管 Mongo 快照、正式态只读正式表、负 ID 首次编辑即 `PLATFORM_OWNED` 物化”的原有模型，删除评审批量转正式、`LEGACY_MANAGED` 持续同步路径及其固定“未请求”审计字段；`V20260730_07` 清理错误副本并恢复 `review_visible_*` 模式隔离。前端类型检查、9 项回归测试和生产构建通过；后端主源码编译、Checkstyle、SpotBugs 0 项及当前 34 项评审读源定向 JUnit 通过，Flyway 不可变性与差异空白检查通过。完整 Maven 测试仍被范围外的 `localhost:15433` 测试库不可达、UTC 时间归一化和统计断言失败阻断。
- [下一步] 容量并发场景同时提交自动增量和页面手动刷新，验证自动运行只在已提交 `RECONCILE` 页边界暂停、手动 publication fence 即使本次 DML 为零仍等待此前目标及必需 generation、原增量随后从同一任务 cursor 恢复；失败任务须能用 `runId/runDbId/taskId/factTaskId/projectionTaskId` 串联。
...
- [阻塞] 2026-08-03：缺陷原因修复的本地真实重建仍未由新代码执行。运行 `497` 虽成功写入 18002 条，但由 13:45 启动的旧后端处理；16:01 复核时它仍是最新运行，6 条项目 9 污染事实未清除。当前代码后端已在 `18080` 健康运行，Chrome 平台页无登录态；须登录后从受保护入口再次重建，才可完成数据库、API 与 CC2026R4 页面验收。
- [风险] 不得用“每次自动增量后全量补偿”替代增量事实修复。当前全量补偿期间定时增量被跳过，补偿完成又会改写 `last_incremental_sync_at`；成功补偿必定追加不可让行的全量事实与快照发布。本地两次完整链路耗时约 7 分 53 秒和 5 分 34 秒，270 万行内网若高频执行会形成持续扫描/写放大并扩大页面事实新鲜度延迟。
- [风险] 本地样本不能替代约 270 万行内网源库的容量基准；2/4/6 worker、删除 batch 和权威范围 batch 的最终值仍须由固定负载重复测量确定。当前实现将删除探测纳入每次普通增量，因此 GitLab 主键查询、运行重叠和手动刷新最坏等待必须在上线前取得真实证据。
- [风险] 新链路只能收敛部署后普通增量或手动刷新实际扫描到的删除；部署前已残留的历史 ODS/事实漂移不会由代码安装本身自动消失。首次部署仍须只执行一次 `FULL_COMPENSATION_SCAN` 及其全量事实/投影发布建立干净基线，此后日常物理删除由普通同步链路收敛，低频全量补偿只保留反熵和灾难恢复职责。
...
- [风险] `check_schema_flyway_drift.py` 当前仍报告 13 张表和 35 个索引仅存在于既有 Flyway 迁移序列；本轮新增结构已与 `schema.sql` 对齐，其余历史基线差异应作为独立工作单元处理。
- [风险] 仓库全量后端和前端套件仍有与本次交接无关的既存失败；发布门禁必须继续区分受影响定向套件、真实 Flyway 验证和全量基线问题，不能把既存失败误报为本次通过。

## 最近验证

- [验证] 2026-07-31：针对计划合并分支事实修复，使用本地项目 325 的 ODS 只读回放确认 1509 条 Issue、145 份最新调研模板，其中 60 份有非空计划合并文本、排除“无/暂无/待定”后 40 份为实际分支值；修复前 `issue_fact` 非空数为 0。后端解析/规则/API/Excel 定向测试 26/26、共享事实分类规则 28/28、前端定向 Vitest 5/5 通过；前端类型检查和生产构建通过，Checkstyle 0 违规，SpotBugs 0 问题。

- [验证] 2026-07-31：增量物理删除与精准发布受影响 42 个测试类 232/232 通过；最终增加自动增量仅在 `RECONCILE` 页为 `TABLE_REFRESH` 让行及同一任务行累计续页后，当前工作树对应 3 类 27/27 通过。Java 21 重新编译 682 个生产源码和 213 个测试源码，Checkstyle 0 违规、SpotBugs 0 问题；前端相关 Vitest 5/5、类型检查和生产构建通过。Flyway 117 个迁移从空库应用成功并锁定，5 个新增迁移 checksum、破坏性迁移审查及自测、Profile 覆盖、跟踪产物、文本空白和 `git diff --check` 通过。schema 漂移缺失为 0，历史基线仍有 13 表/35 索引只存在于 Flyway；运行产物位置门禁只被用户既有 8 个根目录 `.tmp-*.log` 阻断。270 万行固定负载与内网真实删除尚未验收，不计为性能或部署完成。

- [验证] 2026-07-31：最终代码状态下父子影响、断链失败、任务 worker 和事实执行器 30 项测试通过；隔离 PostgreSQL/Flyway 分别验证事实子运行从父运行表任务解析 Issue 目标，以及已有 Issue 标签变化后 `bug_status` 从“未修复”更新为“已修复/完成”、`is_fixed` 从 false 更新为 true。Java 21 完整重编译 644 个生产源码和 204 个测试源码通过，Checkstyle 0 违规、SpotBugs 0 问题，文本空白、跟踪产物及 `git diff --check` 通过。测试端口 `15433` 未启动，未重复运行受既有 source-instance/响应模板断言影响的完整数据库测试类；运行产物位置门禁仍只被根目录既有 8 个 `.tmp-*.log` 阻塞，未移动或删除用户文件。
- [验证] 2026-07-31：本地 PostgreSQL 只读核验确认增量父运行 `484/491` 分别写入 2104/873 行并拥有 48/46 个表任务；事实子运行 `485/492` 的 `parent_run_id` 与 payload 均正确，但旧实现把 `fact_build_tasks.run_id` 直接当成影响来源，查询落到没有表任务的 `485/492`，三类事实均 `SUCCESS/0`。全量事实运行 `489/495` 绕开影响解析并分别重建 8002 条 Issue、10000 条 MR。新老平台源码确认缺陷汇总“已修复”均来自状态标签语义，排除页面口径差异。
- [验证] 2026-07-31：全量补偿后续影响通过调度、提交、镜像分页、让行、截止时间、事实监听/执行代码和本地运行交叉确认。`488 -> 489`、`490 -> 495` 完整链路分别约 7 分 53 秒、5 分 34 秒；`490` 的 1110 个任务扫描 537844 行并为 `491/493` 两次让行，随后 `495` 全量发布 8002 条 Issue 和 10000 条 MR。配置最近增量时间与 `490` 完成时间一致，证实全量补偿会重置增量调度时钟。相关定向 JUnit 53 项通过；未执行写入型同步、补偿或事实重建。
- [验证] 2026-07-29：同步运行时及直接调用者定向套件 160 项通过；运行 owner 唯一性/转移/终态 fencing、首次心跳异常容量归还和表任务 Spring 装配回归补验 19 项通过，Flyway 13 项实库烟测通过。Checkstyle 0 违规、SpotBugs 0 问题、跳过测试生产打包、前端 18 项定向 Vitest、TypeScript 类型检查和生产构建通过。最新 JAR 在本地 PostgreSQL `15432` 完成迁移并于 `18080` 健康启动；管理员 Session 的只读测试连接返回“GitLab PostgreSQL 连接成功”，未提交同步或事实任务。管理员真实链路打开数据镜像设置与表任务抽屉，1440px 桌面四摘要同排、390px 窄屏 2×2 重排，表格保持内部横向滚动且页面无横向溢出，控制台无警告或错误。本地 LDAP 可达但不接受用户提供的 `admin/admin123`，因此 UI 验收使用项目支持的本地开发认证并保持 CSRF 开启；内网 LDAP 发布配置未修改。完整后端套件仍受测试 PostgreSQL `15433` 不可用和既有统计断言失败影响，不计为全套通过。
