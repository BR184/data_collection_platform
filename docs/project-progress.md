...
- CC/DGM 多源镜像隔离
  - 2026-07-01 口径更新：本节记录的是已废弃的历史方案。当前 GitLab 直连镜像只保留 `ods_gitlab_<source_table>` 单套 ODS 表，不再生成或读取来源前缀镜像表；历史测试库可清理后重新导入 GitLab 数据。DGM 后续通过 webhook 或外部导入进入事实层/外部指标表，不恢复 GitLab 直连镜像多表切换。
  ...
  - 当时推荐的新平台方案是用一个平台库模拟老平台的两个数据源命名空间，优先采用来源前缀 ODS 表隔离；该方向现已废弃，事实层仍可继续使用已有的 `source_instance` 字段表示外部来源。
  ...
  - 2026-05-07 已开始落地“一个平台库 + 多 GitLab 镜像源 + 来源实例隔离”方案：
    ...
    - ODS 镜像表当时按来源实例隔离；该方向现已废弃，当前只保留无来源前缀的 `ods_gitlab_<source_table>` 表名。
...
- CC/DGM 多 GitLab 数据镜像源第一阶段落地（历史方案，2026-07-01 已废弃）
  - 2026-07-01 口径更新：本段为历史进展记录，不作为当前实现依据。当前 GitLab 直连镜像不再支持多来源 ODS 表隔离，系统设置也不再提供来源标识/多数据源切换。
  ...
  - 当时 GitLab mirror 导入支持按来源生成隔离表名；该方向现已废弃，当前只保留无来源前缀的 `ods_gitlab_<source_table>` 表名。
