...
export const BI_CHART_EXPLANATIONS = {
  ...
  requirementReviewScatter: '展示每次独立需求评审活动的速率与缺陷密度分布，横轴为评审速率（页/小时），纵轴为缺陷密度（个/页），带色散点标识单次是否达标。存在极端值时轴自动断轴：锯齿带为极值区压缩展示，悬停查看真实值。',

  designReviewQuality: '统计各模块设计评审缺陷密度与评审速率。达标标准：设计评审缺陷密度介于[0.3~0.8]个/页；评审速率无达标要求。',
  designReviewCategories: '统计设计评审发现的问题类别分布及占比。',
  designReviewScatter: '展示每次独立设计评审活动的速率与缺陷密度分布，横轴为评审速率（页/小时），纵轴为缺陷密度（个/页），带色散点标识单次是否达标。存在极端值时轴自动断轴：锯齿带为极值区压缩展示，悬停查看真实值。',
  ...
  codingQualityTrend: '展示按时间周期的代码注释率(%)与人工走查缺陷密度(个/KLOC)双轨演进趋势。折线断开表示当前选择粒度的时间桶无合法注释率观测；存在极端值时轴自动断轴：锯齿带为极值区压缩展示，悬停查看真实值。',
  codingCodeTrend: '展示按日或按周的新增代码量（柱状）与版本累计代码量（折线），单位：行（代码行）。',
  codingSubmissionTrend: '展示按日或按周的代码提交次数（Commit）与合并请求数（MR）演进趋势。',
  codingReviewScatter: '展示每次代码走查的速率与缺陷密度分布，横轴为走查速率（KLOC/小时），纵轴为缺陷密度（个/KLOC）。存在极端值时轴自动断轴：锯齿带为极值区压缩展示，悬停查看真实值。',
