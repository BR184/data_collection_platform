...
import { buildRepairRateChartOption } from './system-test-multi-board';
...
describe('system test multi board chart helpers', () => {
  it('uses ratio display value for repair-rate chart instead of raw numerator', () => {
    const option = buildRepairRateChartOption(boardWithRate('80.00%'));

    expect(firstSeriesData(option)).toEqual([80]);
  });

  it('falls back to numeric value when repair-rate display value is not a percent', () => {
    const option = buildRepairRateChartOption(boardWithRate('80.00', 76.25));
