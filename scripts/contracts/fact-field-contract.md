...
3. Flyway 是创建字段和索引的唯一结构入口；测试通过完整迁移验证建库结果，不维护第二份结构快照。
...
| --- | --- | --- | --- | --- | --- |
| `issue_fact` | `bug_status` | 只提取中文全角 `状态：X` 标签并按老平台格式合并；缺失时写“未设定议题状态” | 按状态成员过滤和聚合，不从议题开闭状态回退 | 客户问题、系统测试的测试状态筛选、标签和导出 | 与 `issue_state/closed_at_source` 独立；规则变化后必须重建议题事实 |
| `issue_fact` | `issue_state` / `closed_at_source` | 保留 GitLab 议题开闭状态与关闭时间 | 判断已关闭、未关闭及相关统计范围 | 议题状态展示、未关闭统计 | 不得写入或反推 `bug_status` |
...
`scripts/check_fact_field_contract.py` 会检查 Flyway 最终字段集合和本文档中的关键事实字段是否一致。本脚本已接入 `scripts/verify-local.ps1` 和 CI 的 `static-guards`，新增事实字段时需要同步更新脚本中的必备字段清单。
...
2. 是否需要补充 Java 生成规则测试。
3. 是否需要补充 SQL 查询或 service 测试。
4. 是否需要更新前端筛选字段、导出字段或明细展示。
5. 是否需要历史数据重建入口。
6. 是否需要更新本文档。
