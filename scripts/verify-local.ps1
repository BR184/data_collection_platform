...
try {
  ...
  git diff --check
  powershell -NoProfile -ExecutionPolicy Bypass -File scripts/gitlab-direct-sync-check.ps1 -DryRun -StartIncrementalSync -RunPageRefreshSmoke -RunReviewDataContextSmoke -SimulateSystemHook -SystemHookSecret dry-run-secret -PollAfterSubmission -RequireCleanMirror
} finally {
  Pop-Location
}
